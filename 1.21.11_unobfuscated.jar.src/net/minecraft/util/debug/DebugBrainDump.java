...
/*  38 */     //   0	7	0	this	Lnet/minecraft/util/debug/DebugBrainDump; } public String name() { return this.name; } public final int hashCode() { // Byte code:
/*     */     //   0: aload_0
...
/*     */   
/*     */   static {
/*  54 */     STREAM_CODEC = net.minecraft.network.codec.StreamCodec.of((output, value) -> value.write(output), DebugBrainDump::new);
/*     */   }
...
/*  62 */         input.readFloat(), 
/*  63 */         input.readUtf(), 
/*  64 */         input.readBoolean(), 
/*  65 */         input.readInt(), 
/*  66 */         input.readList(FriendlyByteBuf::readUtf), 
/*  67 */         input.readList(FriendlyByteBuf::readUtf), 
/*  68 */         input.readList(FriendlyByteBuf::readUtf), 
/*  69 */         input.readList(FriendlyByteBuf::readUtf), (Set<BlockPos>)
/*  70 */         input.readCollection(java.util.HashSet::new, (StreamDecoder)BlockPos.STREAM_CODEC), (Set<BlockPos>)
/*  71 */         input.readCollection(java.util.HashSet::new, (StreamDecoder)BlockPos.STREAM_CODEC));
/*     */   }
/*     */ 
/*     */   
...
/*  88 */     output.writeCollection(this.pois, (net.minecraft.network.codec.StreamEncoder)BlockPos.STREAM_CODEC);
/*  89 */     output.writeCollection(this.potentialPois, (net.minecraft.network.codec.StreamEncoder)BlockPos.STREAM_CODEC);
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 154 */     java.util.Objects.requireNonNull(brain);
/* 155 */     java.util.Objects.requireNonNull(brain); return (Set<BlockPos>)Stream.<MemoryModuleType<GlobalPos>>of(memories).filter(brain::hasMemoryValue).map(brain::getMemory)
/* 156 */       .flatMap(Optional::stream)
/* 157 */       .map(GlobalPos::pos)
/* 158 */       .collect(java.util.stream.Collectors.toSet());
/*     */   }
/*     */   
/*     */   private static List<String> getVillagerGossips(net.minecraft.world.entity.npc.villager.Villager villager) {
/* 162 */     List<String> gossips = new java.util.ArrayList<>();
/* 163 */     villager.getGossips().getGossipEntries().forEach((uuid, entries) -> {
/*     */           String gossipeeName = net.minecraft.network.protocol.game.DebugEntityNameGenerator.getEntityName(uuid);
/*     */           
/*     */           entries.forEach(());
/*     */         });
/*     */     
...
/* 176 */     return body.getBrain().getMemories().entrySet().stream()
/* 177 */       .map(entry -> {
/*     */           MemoryModuleType<?> memoryType = (MemoryModuleType)entry.getKey();
/*     */           
/*     */           Optional<? extends ExpirableValue<?>> optionalExpirableValue = (Optional<? extends ExpirableValue<?>>)entry.getValue();
...
/* 199 */       description = "-";
/*     */     } 
/* 201 */     return net.minecraft.core.registries.BuiltInRegistries.MEMORY_MODULE_TYPE.getKey(memoryType).getPath() + ": " + net.minecraft.core.registries.BuiltInRegistries.MEMORY_MODULE_TYPE.getKey(memoryType).getPath();
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/*     */ 
/*     */   
/*     */   private static String getShortDescription(ServerLevel level, Object obj) {
/*     */     // Byte code:
/*     */     //   0: aload_1
/*     */     //   1: astore_2
/*     */     //   2: iconst_0
...
/* 230 */     return this.pois.contains(poiPos);
/*     */   }
/*     */   
/*     */   public boolean hasPotentialPoi(BlockPos poiPos) {
/* 234 */     return this.potentialPois.contains(poiPos);
/*     */   }
/*     */ }


/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/util/debug/DebugBrainDump.class
