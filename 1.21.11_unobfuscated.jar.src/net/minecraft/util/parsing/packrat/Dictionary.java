...
/* 15 */     Entry<S, T> holder = (Entry<S, T>)this.terms.computeIfAbsent(name, Entry::new);
...
/* 28 */     return put(name, Rule.fromTerm(term, action));
/*    */   }
/*    */   
/*    */   public <T> NamedRule<S, T> put(Atom<T> name, Term<S> term, Rule.SimpleRuleAction<S, T> action) {
/* 32 */     return put(name, Rule.fromTerm(term, action));
/*    */   }
/*    */   
...
/* 44 */     return Objects.<NamedRule<S, T>>requireNonNull((Entry)this.terms.get(name), () -> "No rule called " + String.valueOf(name));
/*    */   }
/*    */   
/*    */   public <T> NamedRule<S, T> forward(Atom<T> name) {
/* 48 */     return getOrCreateEntry(name);
/*    */   }
/*    */ 
/*    */   
/*    */   private <T> Entry<S, T> getOrCreateEntry(Atom<T> name) {
/* 53 */     return (Entry<S, T>)this.terms.computeIfAbsent(name, Entry::new);
/*    */   }
/*    */   
/*    */   public <T> Term<S> named(Atom<T> name) {
/* 57 */     return new Reference<>(getOrCreateEntry(name), name);
/*    */   }
/*    */   
/*    */   public <T> Term<S> namedWithAlias(Atom<T> nameToParse, Atom<T> nameToStore) {
/* 61 */     return new Reference<>(getOrCreateEntry(nameToParse), nameToStore);
/*    */   }
...
/* 67 */       T result = state.parse(this.ruleToParse);
/* 68 */       if (result == null) {
/* 69 */         return false;
/*    */       }
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/util/parsing/packrat/Dictionary.class
