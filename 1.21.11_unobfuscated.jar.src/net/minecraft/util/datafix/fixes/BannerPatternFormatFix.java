...
/* 58 */     super(outputSchema, false, "BannerPatternFormatFix", References.BLOCK_ENTITY, "minecraft:banner");
/*    */   }
/*    */ 
/*    */   
/*    */   protected Typed<?> fix(Typed<?> entity) {
/* 63 */     return entity.update(DSL.remainderFinder(), BannerPatternFormatFix::fixTag);
/*    */   }
/*    */   
/*    */   private static Dynamic<?> fixTag(Dynamic<?> tag) {
/* 67 */     return tag.renameAndFixField("Patterns", "patterns", patterns -> patterns.createList(patterns.asStream().map(BannerPatternFormatFix::fixLayer)));
/*    */   }
/*    */ 
/*    */ 
/*    */   
...
/*    */           Objects.requireNonNull(pattern); return DataFixUtils.orElse(pattern.asString().map(()).map(pattern::createString).result(), pattern);
/*    */         });
/* 76 */     dynamic = dynamic.set("color", dynamic.createString(ExtraDataFixUtils.dyeColorIdToName(dynamic.get("Color").asInt(0))));
/* 77 */     dynamic = dynamic.remove("Color");
/* 78 */     return dynamic;
/*    */   }
/*    */ }


/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/util/datafix/fixes/BannerPatternFormatFix.class
