...
/*     */       try {
/*  71 */         Path created = Files.createDirectory(fullPath, (FileAttribute<?>[])new FileAttribute[0]);
...
/*  86 */     for (Path part : (Iterable<Path>)path) {
...
/*  96 */     return !RESERVED_WINDOWS_FILENAMES.matcher(name).matches();
/*     */   }
/*     */   
...
/* 111 */     return FilenameUtils.getFullPath(filename).replace(File.separator, "/");
/*     */   }
/*     */   
/*     */   public static String normalizeResourcePath(String filename) {
/* 115 */     return FilenameUtils.normalize(filename).replace(File.separator, "/");
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 128 */     if (segmentEnd == -1) {
/* 129 */       switch (path) { case "": case ".": case "..":
/*     */         
/*     */         default:
/* 132 */           if (!containsAllowedCharactersOnly(path));
/*     */           break; }
/*     */       
/* 135 */       return DataResult.success(List.of(path));
/*     */     } 
/*     */ 
/*     */ 
/*     */     
...
/*     */       
/*     */       default:
/* 178 */         rest = new String[size - 1];
/* 179 */         for (i = 1; i < size; i++)
/* 180 */           rest[i - 1] = segments.get(i);  }
/*     */     
/* 182 */     return root.resolve(root.getFileSystem().getPath(segments.get(0), rest));
/*     */   }
/*     */ 
/*     */ 
/*     */   
/*     */   private static boolean containsAllowedCharactersOnly(String segment) {
/* 188 */     return STRICT_PATH_SEGMENT_CHECK.matcher(segment).matches();
/*     */   }
/*     */   
/*     */   public static boolean isValidPathSegment(String segment) {
/* 192 */     return (!segment.equals("..") && !segment.equals(".") && containsAllowedCharactersOnly(segment));
/*     */   }
/*     */   
...
/* 208 */     Files.createDirectories(Files.exists(dir, new java.nio.file.LinkOption[0]) ? dir.toRealPath(new java.nio.file.LinkOption[0]) : dir, (FileAttribute<?>[])new FileAttribute[0]);
/*     */   }
/*     */ }


/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/util/FileUtil.class
