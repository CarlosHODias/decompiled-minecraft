...
/*  53 */     brain.addActivity(Activity.CORE, 0, ImmutableList.of(new Swim<Creaking>(0.8F)
/*     */           {
/*     */             protected boolean checkExtraStartConditions(ServerLevel level, Creaking body)
/*     */             {
/*  57 */               return (body.canMove() && super.checkExtraStartConditions(level, (LivingEntity)body));
/*     */             }
...
/*  70 */               Pair.of(RandomStroll.stroll(0.3F), 2), 
/*  71 */               Pair.of(SetWalkTargetFromLookTarget.create(0.3F, 3), 2), 
/*  72 */               Pair.of(new DoNothing(30, 60), 1)))));
/*     */   }
/*     */ 
/*     */ 
/*     */   
/*     */   static void initFightActivity(Creaking body, Brain<Creaking> brain) {
/*  78 */     brain.addActivityWithConditions(Activity.FIGHT, 10, ImmutableList.of(
...
/*  82 */         (Set)ImmutableSet.of(Pair.of(MemoryModuleType.ATTACK_TARGET, MemoryStatus.VALUE_PRESENT)));
/*     */   }
/*     */   
...
/*  87 */     return (Boolean)visibleAttackablePlayers.<Boolean>map(players -> { if (target instanceof Player) { Player player = (Player)target; if (players.contains(player)); }  return false; }).orElse(false);
/*     */   }
/*     */   
...
/*     */     
/*  99 */     brain.setCoreActivities((Set)ImmutableSet.of(Activity.CORE));
...
/*     */     } else {
/* 109 */       creaking.getBrain().setActiveActivityToFirstValid((List)ImmutableList.of(Activity.FIGHT, Activity.IDLE));
/*     */     } 
/*     */   }
/*     */ }


/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/entity/monster/creaking/CreakingAi.class
