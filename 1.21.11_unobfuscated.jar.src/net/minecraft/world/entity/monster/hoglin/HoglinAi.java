...
/*     */     
/*  73 */     brain.setCoreActivities((Set)ImmutableSet.of(Activity.CORE));
...
/*  80 */     brain.addActivity(Activity.CORE, 0, ImmutableList.of(new LookAtTargetSink(45, 90), new MoveToTargetSink()));
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 100 */     brain.addActivityAndRemoveMemoryWhenStopped(Activity.FIGHT, 10, ImmutableList.of(
...
/* 112 */     brain.addActivityAndRemoveMemoryWhenStopped(Activity.AVOID, 10, ImmutableList.of(
...
/* 122 */           Pair.of(RandomStroll.stroll(0.4F), 2), 
/* 123 */           Pair.of(SetWalkTargetFromLookTarget.create(0.4F, 3), 2), 
/* 124 */           Pair.of(new DoNothing(30, 60), 1)));
/*     */   }
/*     */ 
/*     */   
...
/*     */     
/* 131 */     Activity oldActivity = brain.getActiveNonCoreActivity().orElse(null);
/*     */ 
/*     */     
...
/*     */ 
/*     */     
/* 140 */     Activity newActivity = brain.getActiveNonCoreActivity().orElse(null);
...
/* 165 */     getVisibleAdultHoglins(body).forEach(hoglin -> retreatFromNearestTarget(hoglin, target));
/*     */   }
/*     */   
...
/* 172 */     nearest = BehaviorUtils.getNearestTarget((LivingEntity)body, brain.getMemory(MemoryModuleType.AVOID_TARGET), nearest);
/* 173 */     nearest = BehaviorUtils.getNearestTarget((LivingEntity)body, brain.getMemory(MemoryModuleType.ATTACK_TARGET), nearest);
/*     */     
...
/* 196 */     return (repellentPos.isPresent() && ((BlockPos)repellentPos.get()).closerThan((Vec3i)pos, 8.0D));
/*     */   }
/*     */   
/*     */   private static boolean wantsToStopFleeing(Hoglin body) {
/* 200 */     return (body.isAdult() && !piglinsOutnumberHoglins(body));
/*     */   }
/*     */   
...
/*     */     }
/*     */     
/* 208 */     int piglinCount = (Integer)body.getBrain().getMemory(MemoryModuleType.VISIBLE_ADULT_PIGLIN_COUNT).orElse(0);
/* 209 */     int hoglinCount = (Integer)body.getBrain().getMemory(MemoryModuleType.VISIBLE_ADULT_HOGLIN_COUNT).orElse(0) + 1;
...
/*     */       return;
/*     */     }
/* 235 */     if (BehaviorUtils.isOtherTargetMuchFurtherAwayThanCurrentAttackTarget((LivingEntity)body, attacker, 4.0D)) {
/*     */       return;
/*     */     }
/*     */     
/* 239 */     if (!Sensor.isEntityAttackable(level, (LivingEntity)body, attacker)) {
/*     */       return;
/*     */     }
/*     */     
...
/* 256 */     getVisibleAdultHoglins(body).forEach(hoglin -> setAttackTargetIfCloserThanCurrent(hoglin, target));
/*     */   }
/*     */   
...
/* 265 */     LivingEntity nearest = BehaviorUtils.getNearestTarget((LivingEntity)body, currentTarget, newTarget);
/* 266 */     setAttackTarget(body, nearest);
/*     */   }
/*     */   
/*     */   public static Optional<SoundEvent> getSoundForCurrentActivity(Hoglin body) {
/* 270 */     return body.getBrain().getActiveNonCoreActivity().map(activity -> getSoundForActivity(body, activity));
/*     */   }
/*     */   
...
/* 286 */     return (List<Hoglin>)body.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_ADULT_HOGLINS).orElse(ImmutableList.of());
/*     */   }
/*     */   
/*     */   private static boolean isNearRepellent(Hoglin body) {
/* 290 */     return body.getBrain().hasMemoryValue(MemoryModuleType.NEAREST_REPELLENT);
/*     */   }
/*     */   
/*     */   private static boolean isBreeding(Hoglin body) {
/* 294 */     return body.getBrain().hasMemoryValue(MemoryModuleType.BREED_TARGET);
/*     */   }
/*     */   
/*     */   protected static boolean isPacified(Hoglin body) {
/* 298 */     return body.getBrain().hasMemoryValue(MemoryModuleType.PACIFIED);
/*     */   }
/*     */ }


/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/entity/monster/hoglin/HoglinAi.class
