...
/*  43 */   private static final int INHALING_DURATION_TICKS = Math.round(10.0F); private static final int JUMP_COOLDOWN_TICKS = 10;
...
/*     */ 
/*     */     
/*  69 */     if (Swim.shouldSwim((Mob)breeze)) {
/*  70 */       return false;
/*     */     }
/*     */     
...
/*     */     }
/*     */     
/*  77 */     LivingEntity attackTarget = breeze.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).orElse(null);
...
/*     */     }
/*     */     
/*  95 */     BlockPos targetPos = snapToSurface((LivingEntity)breeze, BreezeUtil.randomPointBehindTarget(attackTarget, breeze.getRandom()));
...
/* 116 */     return canRun(level, breeze);
/*     */   }
/*     */ 
/*     */   
/*     */   protected boolean canStillUse(ServerLevel level, Breeze breeze, long timestamp) {
/* 121 */     return (breeze.getPose() != Pose.STANDING && !breeze.getBrain().hasMemoryValue(MemoryModuleType.BREEZE_JUMP_COOLDOWN));
/*     */   }
/*     */ 
/*     */   
...
/* 131 */     level.playSound(null, (Entity)breeze, SoundEvents.BREEZE_CHARGE, SoundSource.HOSTILE, 1.0F, 1.0F);
...
/*     */       
/* 144 */       Vec3 velocityVector = breeze.getBrain().getMemory(MemoryModuleType.BREEZE_JUMP_TARGET)
/* 145 */         .flatMap(targetPos -> calculateOptimalJumpVector(breeze, breeze.getRandom(), Vec3.atBottomCenterOf((Vec3i)targetPos)))
/* 146 */         .orElse(null);
/*     */       
...
/* 184 */     return (breeze.getBrain().getMemory(MemoryModuleType.BREEZE_JUMP_INHALING).isEmpty() && breeze.getPose() == Pose.INHALING);
/*     */   }
/*     */   
...
/* 195 */     ClipContext collisionBelow = new ClipContext(target, target.relative(Direction.DOWN, 10.0D), ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, (Entity)entity);
/* 196 */     BlockHitResult blockHitResult1 = entity.level().clip(collisionBelow);
/* 197 */     if (blockHitResult1.getType() == HitResult.Type.BLOCK) {
/* 198 */       return BlockPos.containing((Position)blockHitResult1.getLocation()).above();
/*     */     }
/*     */     
/* 201 */     ClipContext collisionAbove = new ClipContext(target, target.relative(Direction.UP, 10.0D), ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, (Entity)entity);
/* 202 */     BlockHitResult blockHitResult2 = entity.level().clip(collisionAbove);
/* 203 */     if (blockHitResult2.getType() == HitResult.Type.BLOCK) {
/* 204 */       return BlockPos.containing((Position)blockHitResult2.getLocation()).above();
/*     */     }
/* 206 */     return null;
/*     */   }
/*     */   
/*     */   private static boolean outOfAggroRange(Breeze breeze, LivingEntity attackTarget) {
/* 210 */     return !attackTarget.closerThan((Entity)breeze, breeze.getAttributeValue(Attributes.FOLLOW_RANGE));
/*     */   }
/*     */   
/*     */   private static boolean tooCloseForJump(Breeze breeze, LivingEntity attackTarget) {
/* 214 */     return (attackTarget.distanceTo((Entity)breeze) - 4.0F <= 0.0F);
/*     */   }
/*     */   
...
/*     */     
/* 236 */     for (Iterator<Integer> iterator = allowedAngles.iterator(); iterator.hasNext(); ) { int angle = (Integer)iterator.next();
/* 237 */       float maxJumpVelocity = 0.058333334F * (float)body.getAttributeValue(Attributes.FOLLOW_RANGE);
/*     */       
/* 239 */       Optional<Vec3> velocityVector = LongJumpUtil.calculateJumpVectorForAngle((Mob)body, targetPos, maxJumpVelocity, angle, false);
/*     */       
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/entity/monster/breeze/LongJump.class
