...
/* 30 */     return mob.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).orElse(null);
/*    */   }
/*    */   
/*    */   private boolean ableToAttack(PathfinderMob mob) {
/* 34 */     return (getTarget(mob) != null && mob.getMainHandItem().has(DataComponents.KINETIC_WEAPON));
/*    */   }
/*    */ 
/*    */   
...
/* 63 */     body.getBrain().setMemory(MemoryModuleType.SPEAR_FLEEING_TIME, 0);
/* 64 */     super.start(level, body, timestamp);
/*    */   }
/*    */ 
/*    */   
/*    */   protected boolean canStillUse(ServerLevel level, PathfinderMob body, long timestamp) {
/* 69 */     return ((Integer)body.getBrain().getMemory(MemoryModuleType.SPEAR_FLEEING_TIME).orElse(100) < 100 && 
/* 70 */       body.getBrain().getMemory(MemoryModuleType.SPEAR_FLEEING_POSITION).isPresent() && 
...
/*    */     
/* 81 */     mob.getBrain().setMemory(MemoryModuleType.LOOK_TARGET, new EntityTracker((Entity)target, true));
/*    */     
/* 83 */     mob.getBrain().setMemory(MemoryModuleType.SPEAR_FLEEING_TIME, (Integer)mob.getBrain().getMemory(MemoryModuleType.SPEAR_FLEEING_TIME).orElse(0) + 1);
/*    */     
/* 85 */     mob.getBrain().getMemory(MemoryModuleType.SPEAR_FLEEING_POSITION).ifPresent(fleePos -> mob.getNavigation().moveTo(mob.x, mob.y, mob.z, mob * this.speedModifierWhenRepositioning));
/*    */   }
/*    */ 
/*    */ 
/*    */ 
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/entity/ai/behavior/SpearRetreat.class
