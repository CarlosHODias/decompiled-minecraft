...
/*  42 */     super(
/*  43 */         (Map<MemoryModuleType<?>, MemoryStatus>)ImmutableMap.of(MemoryModuleType.CANT_REACH_WALK_TARGET_SINCE, MemoryStatus.REGISTERED, MemoryModuleType.PATH, MemoryStatus.VALUE_ABSENT, MemoryModuleType.WALK_TARGET, MemoryStatus.VALUE_PRESENT), minTimeout, maxTimeout);
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/*  60 */     WalkTarget walkTarget = brain.getMemory(MemoryModuleType.WALK_TARGET).get();
/*     */     
...
/*  84 */     boolean isSpectator = (Boolean)walkTarget.<Boolean>map(MoveToTargetSink::isWalkTargetSpectator).orElse(false);
/*     */     
/*  86 */     PathNavigation navigation = body.getNavigation();
/*  87 */     return (!navigation.isDone() && walkTarget.isPresent() && !reachedTarget(body, walkTarget.get()) && !isSpectator);
/*     */   }
/*     */ 
/*     */   
/*     */   protected void stop(ServerLevel level, Mob body, long timestamp) {
/*  92 */     if (body.getBrain().hasMemoryValue(MemoryModuleType.WALK_TARGET) && !reachedTarget(body, body.getBrain().getMemory(MemoryModuleType.WALK_TARGET).get()) && body.getNavigation().isStuck())
/*     */     {
...
/*     */     }
/*     */     
/* 122 */     WalkTarget walkTarget = brain.getMemory(MemoryModuleType.WALK_TARGET).get();
/*     */     
/* 124 */     if (walkTarget.getTarget().currentBlockPosition().distSqr((Vec3i)this.lastTargetPos) > 4.0D && 
...
/* 146 */         brain.setMemory(MemoryModuleType.CANT_REACH_WALK_TARGET_SINCE, timestamp);
/*     */       } 
/*     */       
...
/*     */       }
/*     */       
/* 153 */       Vec3 partialStep = DefaultRandomPos.getPosTowards((PathfinderMob)body, 10, 7, Vec3.atBottomCenterOf((Vec3i)targetPos), 1.5707963705062866D);
...
/* 163 */     return (walkTarget.getTarget().currentBlockPosition().distManhattan((Vec3i)body.blockPosition()) <= walkTarget.getCloseEnoughDist());
/*     */   }
/*     */   
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/entity/ai/behavior/MoveToTargetSink.class
