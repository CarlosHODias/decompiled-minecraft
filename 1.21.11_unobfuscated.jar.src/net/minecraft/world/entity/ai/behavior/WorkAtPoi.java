...
/*    */     
/* 43 */     GlobalPos target = body.getBrain().getMemory(MemoryModuleType.JOB_SITE).get();
/* 44 */     return (target.dimension() == level.dimension() && target.pos().closerToCenterThan((Position)body.position(), 1.73D));
/*    */   }
/*    */ 
/*    */   
/*    */   protected void start(ServerLevel level, Villager body, long timestamp) {
/* 49 */     Brain<Villager> brain = body.getBrain();
/* 50 */     brain.setMemory(MemoryModuleType.LAST_WORKED_AT_POI, timestamp);
/* 51 */     brain.getMemory(MemoryModuleType.JOB_SITE).ifPresent(globalPos -> brain.setMemory(MemoryModuleType.LOOK_TARGET, new BlockPosTracker(globalPos.pos())));
/*    */ 
/*    */ 
/*    */     
...
/*    */     }
/*    */     
/* 73 */     GlobalPos target = jobSiteMemory.get();
/* 74 */     return (target.dimension() == level.dimension() && 
/* 75 */       target.pos().closerToCenterThan((Position)body.position(), 1.73D));
/*    */   }
/*    */ }


/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/entity/ai/behavior/WorkAtPoi.class
