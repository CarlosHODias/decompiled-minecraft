...
/*  53 */     super((Map<MemoryModuleType<?>, MemoryStatus>)ImmutableMap.of(MemoryModuleType.CHARGE_COOLDOWN_TICKS, MemoryStatus.VALUE_ABSENT, MemoryModuleType.ATTACK_TARGET, MemoryStatus.VALUE_PRESENT));
/*     */ 
/*     */ 
/*     */     
...
/*  70 */     return body.getBrain().hasMemoryValue(MemoryModuleType.ATTACK_TARGET);
/*     */   }
/*     */ 
/*     */   
...
/*  78 */       return false;
/*     */     }
/*  80 */     LivingEntity attackTarget = attackCandidate.get();
/*     */ 
/*     */     
...
/*     */     }
/*     */     
/*  94 */     if (!body.hasLineOfSight((Entity)attackTarget)) {
/*  95 */       return false;
/*     */     }
...
/* 104 */     LivingEntity attackCandidate = brain.getMemory(MemoryModuleType.ATTACK_TARGET).get();
...
/* 115 */     LivingEntity attackTarget = brain.getMemory(MemoryModuleType.ATTACK_TARGET).orElseThrow();
/* 116 */     body.lookAt((Entity)attackTarget, 360.0F, 360.0F);
/* 117 */     body.setDeltaMovement(this.chargeVelocityVector);
/*     */     
/* 119 */     List<LivingEntity> collidingEntities = new ArrayList<>(1);
/* 120 */     level.getEntities(EntityTypeTest.forClass(LivingEntity.class), body.getBoundingBox(), e -> this.chargeTargeting.test(level, (LivingEntity)level, body), collidingEntities, 1);
/* 121 */     if (!collidingEntities.isEmpty()) {
/* 122 */       LivingEntity closestAttackTarget = collidingEntities.get(0);
/* 123 */       if (body.hasPassenger((Entity)closestAttackTarget)) {
/*     */         return;
/*     */       }
...
/* 133 */     DamageSource damageSource = level.damageSources().mobAttack((LivingEntity)body);
/* 134 */     float damage = (float)body.getAttributeValue(Attributes.ATTACK_DAMAGE);
/* 135 */     if (target.hurtServer(level, damageSource, damage)) {
/* 136 */       EnchantmentHelper.doPostAttackEffects(level, (Entity)target, damageSource);
/*     */     }
/*     */   }
/*     */ 
/*     */   
...
/* 146 */     body.causeExtraKnockback((Entity)target, speedFactor * this.knockbackForce, body.getDeltaMovement());
/*     */   }
/*     */ 
/*     */   
/*     */   protected void stop(ServerLevel level, Animal body, long timestamp) {
/* 151 */     body.getBrain().setMemory(MemoryModuleType.CHARGE_COOLDOWN_TICKS, this.timeBetweenAttacks);
/* 152 */     body.getBrain().eraseMemory(MemoryModuleType.ATTACK_TARGET);
/*     */   }
/*     */ }


/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/entity/ai/behavior/ChargeAttack.class
