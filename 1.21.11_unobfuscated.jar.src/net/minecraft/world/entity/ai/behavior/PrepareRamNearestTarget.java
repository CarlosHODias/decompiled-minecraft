...
/*  74 */       .flatMap(livingEntities -> body.findClosest(()))
/*  75 */       .ifPresent(livingEntity -> chooseRamPosition(body, body));
/*     */   }
/*     */ 
/*     */   
...
/*  82 */       level.broadcastEntityEvent((Entity)body, (byte)59);
/*  83 */       brain.setMemory(MemoryModuleType.RAM_COOLDOWN_TICKS, this.getCooldownOnFail.applyAsInt(body));
/*     */     } 
/*     */   }
/*     */ 
/*     */   
/*     */   protected boolean canStillUse(ServerLevel level, PathfinderMob body, long timestamp) {
/*  89 */     return (this.ramCandidate.isPresent() && ((RamCandidate)
/*  90 */       this.ramCandidate.get()).getTarget().isAlive());
/*     */   }
/*     */ 
/*     */   
...
/* 100 */     body.getBrain().setMemory(MemoryModuleType.LOOK_TARGET, new EntityTracker((Entity)((RamCandidate)this.ramCandidate.get()).getTarget(), true));
/*     */     
...
/* 104 */       level.broadcastEntityEvent((Entity)body, (byte)59);
/* 105 */       body.getNavigation().stop();
/* 106 */       chooseRamPosition((PathfinderMob)body, ((RamCandidate)this.ramCandidate.get()).target);
/*     */     } else {
/* 108 */       BlockPos startRamPos = body.blockPosition();
/* 109 */       if (startRamPos.equals(((RamCandidate)this.ramCandidate.get()).getStartPosition())) {
/* 110 */         level.broadcastEntityEvent((Entity)body, (byte)58);
/* 111 */         if (this.reachedRamPositionTimestamp.isEmpty()) {
/* 112 */           this.reachedRamPositionTimestamp = Optional.of(timestamp);
/*     */         }
/* 114 */         if (timestamp - (Long)this.reachedRamPositionTimestamp.get() >= this.ramPrepareTime) {
...
/* 116 */           level.playSound(null, (Entity)body, this.getPrepareRamSound.apply(body), SoundSource.NEUTRAL, 1.0F, body.getVoicePitch());
...
/*     */     
/* 128 */     return Vec3.atBottomCenterOf((Vec3i)targetPos).add(xOffset, 0.0D, zOffset);
/*     */   }
/*     */   
...
/* 140 */     for (Direction direction : (Iterable<Direction>)Direction.Plane.HORIZONTAL) {
/* 141 */       walkablePosFurthestAwayFromTarget.set((Vec3i)targetPos);
/* 142 */       for (int distance = 0; distance < this.maxRamDistance; distance++) {
/* 143 */         if (!isWalkableBlock(body, (BlockPos)walkablePosFurthestAwayFromTarget.move(direction))) {
/* 144 */           walkablePosFurthestAwayFromTarget.move(direction.getOpposite());
/*     */           
/*     */           break;
/*     */         } 
/*     */       } 
/* 149 */       if (walkablePosFurthestAwayFromTarget.distManhattan((Vec3i)targetPos) >= this.minRamDistance) {
...
/* 166 */     return (body.getNavigation().isStableDestination(targetPos) && 
/* 167 */       body.getPathfindingMalus(WalkNodeEvaluator.getPathTypeStatic((Mob)body, targetPos)) == 0.0F);
/*     */   }
/*     */   
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/entity/ai/behavior/PrepareRamNearestTarget.class
