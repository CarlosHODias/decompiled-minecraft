...
/*  52 */     super((Map<MemoryModuleType<?>, MemoryStatus>)ImmutableMap.of(MemoryModuleType.RAM_COOLDOWN_TICKS, MemoryStatus.VALUE_ABSENT, MemoryModuleType.RAM_TARGET, MemoryStatus.VALUE_PRESENT), 200);
/*     */ 
/*     */ 
/*     */ 
/*     */     
...
/*  69 */     return body.getBrain().hasMemoryValue(MemoryModuleType.RAM_TARGET);
/*     */   }
/*     */ 
/*     */   
/*     */   protected boolean canStillUse(ServerLevel level, Goat body, long timestamp) {
/*  74 */     return body.getBrain().hasMemoryValue(MemoryModuleType.RAM_TARGET);
/*     */   }
/*     */ 
/*     */   
...
/*  81 */     Vec3 ramTargetPos = brain.getMemory(MemoryModuleType.RAM_TARGET).get();
/*     */ 
/*     */     
...
/*  90 */     List<LivingEntity> nearbyEntities = level.getNearbyEntities(LivingEntity.class, this.ramTargeting, (LivingEntity)body, body.getBoundingBox());
/*     */     
/*  92 */     Brain<?> brain = body.getBrain();
/*  93 */     if (!nearbyEntities.isEmpty()) {
/*  94 */       LivingEntity ramTarget = nearbyEntities.get(0);
/*  95 */       DamageSource damageSource = level.damageSources().noAggroMobAttack((LivingEntity)body);
...
/*  98 */         EnchantmentHelper.doPostAttackEffects(level, (Entity)ramTarget, damageSource);
/*     */       }
/*     */       
...
/*     */       
/* 106 */       DamageSource source = level.damageSources().mobAttack((LivingEntity)body);
...
/* 112 */       level.playSound(null, (Entity)body, this.getImpactSound.apply(body), SoundSource.NEUTRAL, 1.0F, 1.0F);
/* 113 */     } else if (hasRammedHornBreakingBlock(level, body)) {
/* 114 */       level.playSound(null, (Entity)body, this.getImpactSound.apply(body), SoundSource.NEUTRAL, 1.0F, 1.0F);
/* 115 */       boolean dropped = body.dropHorn();
/* 116 */       if (dropped) {
/* 117 */         level.playSound(null, (Entity)body, this.getHornBreakSound.apply(body), SoundSource.NEUTRAL, 1.0F, 1.0F);
/*     */       }
...
/* 123 */       boolean lostOrReachedTarget = (walkTarget.isEmpty() || 
/* 124 */         ramTarget.isEmpty() || ((WalkTarget)
/* 125 */         walkTarget.get()).getTarget().currentPosition().closerThan((Position)ramTarget.get(), 0.25D));
...
/* 134 */     BlockPos facingBlockPosition = BlockPos.containing((Position)body.position().add(horizontalMovementNormalized));
...
/* 139 */     level.broadcastEntityEvent((Entity)body, (byte)59);
/* 140 */     body.getBrain().setMemory(MemoryModuleType.RAM_COOLDOWN_TICKS, ((UniformInt)this.getTimeBetweenRams.apply(body)).sample(level.random));
/* 141 */     body.getBrain().eraseMemory(MemoryModuleType.RAM_TARGET);
/*     */   }
/*     */ }


/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/entity/ai/behavior/RamTarget.class
