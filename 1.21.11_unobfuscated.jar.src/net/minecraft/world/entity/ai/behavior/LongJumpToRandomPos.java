...
/*  37 */   private static final List<Integer> ALLOWED_ANGLES = Lists.newArrayList((Object[])new Integer[] { 65, 70, 75, 80 });
/*     */   
...
/*  53 */     this(timeBetweenLongJumps, maxLongJumpHeight, maxLongJumpWidth, maxJumpVelocityMultiplier, getJumpSound, LongJumpToRandomPos::defaultAcceptableLandingSpot);
/*     */   }
/*     */   
...
/*  59 */     return (level.getBlockState(below).isSolidRender() && 
/*  60 */       body.getPathfindingMalus(WalkNodeEvaluator.getPathTypeStatic((Mob)body, targetPos)) == 0.0F);
/*     */   }
/*     */   
...
/*  64 */     super((Map<MemoryModuleType<?>, MemoryStatus>)com.google.common.collect.ImmutableMap.of(MemoryModuleType.LOOK_TARGET, MemoryStatus.REGISTERED, MemoryModuleType.LONG_JUMP_COOLDOWN_TICKS, MemoryStatus.VALUE_ABSENT, MemoryModuleType.LONG_JUMP_MID_JUMP, MemoryStatus.VALUE_ABSENT), 200);
/*     */ 
/*     */ 
/*     */ 
/*     */ 
...
/*  81 */     if (!canStart) {
/*  82 */       body.getBrain().setMemory(MemoryModuleType.LONG_JUMP_COOLDOWN_TICKS, this.timeBetweenLongJumps.sample(level.random) / 2);
/*     */     }
/*  84 */     return canStart;
/*     */   }
/*     */ 
...
/*  89 */     boolean isValid = (this.initialPosition.isPresent() && ((Vec3)
/*  90 */       this.initialPosition.get()).equals(body.position()) && this.findJumpTries > 0 && 
/*     */       
/*  92 */       !body.isInWater() && (this.chosenJump != null || 
/*  93 */       !this.jumpCandidates.isEmpty()));
/*     */     
/*  95 */     if (!isValid && body.getBrain().getMemory(MemoryModuleType.LONG_JUMP_MID_JUMP).isEmpty()) {
/*  96 */       body.getBrain().setMemory(MemoryModuleType.LONG_JUMP_COOLDOWN_TICKS, this.timeBetweenLongJumps.sample(level.random) / 2);
...
/*     */ 
/*     */       
/* 123 */       .jumpCandidates = (List<PossibleJump>)BlockPos.betweenClosedStream(mobX - this.maxLongJumpWidth, mobY - this.maxLongJumpHeight, mobZ - this.maxLongJumpWidth, mobX + this.maxLongJumpWidth, mobY + this.maxLongJumpHeight, mobZ + this.maxLongJumpWidth).filter(pos -> !pos.equals(mobPos)).map(pos -> new PossibleJump(pos.immutable(), Mth.ceil(mobPos.distSqr((Vec3i)pos)))).collect(Collectors.toCollection(Lists::newArrayList));
/*     */   }
/*     */ 
/*     */   
...
/* 130 */         body.setYRot(((Mob)body).yBodyRot);
...
/*     */         
/* 138 */         body.getBrain().setMemory(MemoryModuleType.LONG_JUMP_MID_JUMP, true);
/* 139 */         level.playSound(null, (Entity)body, this.getJumpSound.apply(body), SoundSource.NEUTRAL, 1.0F, 1.0F);
/*     */       } 
/*     */     } else {
...
/*     */       }
/*     */       
/* 154 */       PossibleJump position = optionalPosition.get();
/*     */       
...
/*     */       }
/*     */       
/* 161 */       Vec3 targetPosition = Vec3.atCenterOf((Vec3i)targetPos);
/* 162 */       Vec3 jumpVector = calculateOptimalJumpVector((Mob)body, targetPosition);
...
/* 182 */     java.util.Objects.requireNonNull(this.jumpCandidates); randomItem.ifPresent(this.jumpCandidates::remove);
...
/*     */     
/* 203 */     for (Iterator<Integer> iterator = allowedAngles.iterator(); iterator.hasNext(); ) { int angle = (Integer)iterator.next();
/*     */       
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/entity/ai/behavior/LongJumpToRandomPos.class
