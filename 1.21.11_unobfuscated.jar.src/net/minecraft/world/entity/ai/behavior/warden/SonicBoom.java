...
/*     */   
/*     */   public SonicBoom() {
/*  32 */     super((Map)ImmutableMap.of(MemoryModuleType.ATTACK_TARGET, MemoryStatus.VALUE_PRESENT, MemoryModuleType.SONIC_BOOM_COOLDOWN, MemoryStatus.VALUE_ABSENT, MemoryModuleType.SONIC_BOOM_SOUND_COOLDOWN, MemoryStatus.REGISTERED, MemoryModuleType.SONIC_BOOM_SOUND_DELAY, MemoryStatus.REGISTERED), DURATION);
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/*  42 */     return body.closerThan(body.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).get(), 15.0D, 20.0D);
/*     */   }
/*     */ 
/*     */   
...
/*  53 */     body.getBrain().setMemoryWithExpiry(MemoryModuleType.ATTACK_COOLING_DOWN, true, DURATION);
/*     */     
/*  55 */     body.getBrain().setMemoryWithExpiry(MemoryModuleType.SONIC_BOOM_SOUND_DELAY, Unit.INSTANCE, TICKS_BEFORE_PLAYING_SOUND);
/*     */     
/*  57 */     level.broadcastEntityEvent((Entity)body, (byte)62);
...
/*  63 */     body.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).ifPresent(target -> body.getLookControl().setLookAt(target.position()));
/*     */ 
/*     */ 
/*     */     
...
/*  74 */       .filter(target -> body.closerThan((Entity)target, 15.0D, 20.0D))
/*  75 */       .ifPresent(target -> {
/*     */           Vec3 source = body.position().add(body.getAttachments().get(EntityAttachment.WARDEN_CHEST, 0, body.getYRot())), delta = target.getEyePosition().subtract(source), normalize = delta.normalize();
/*     */           int steps = Mth.floor(delta.length()) + 7;
/*     */           for (int i = 1; i < steps; i++) {
/*     */             Vec3 particlePos = source.add(normalize.scale(i));
/*     */             level.sendParticles((ParticleOptions)ParticleTypes.SONIC_BOOM, particlePos.x, particlePos.y, particlePos.z, 1, 0.0D, 0.0D, 0.0D, 0.0D);
/*     */           } 
/*     */           body.playSound(SoundEvents.WARDEN_SONIC_BOOM, 3.0F, 1.0F);
/*     */           if (target.hurtServer(level, level.damageSources().sonicBoom((Entity)body), 10.0F)) {
/*     */             double knockbackVertical = 0.5D * (1.0D - target.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE)), knockbackHorizontal = 2.5D * (1.0D - target.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE));
...
/*  99 */     setCooldown((LivingEntity)body, 40);
/*     */   }
/*     */   
/*     */   public static void setCooldown(LivingEntity body, int cooldown) {
/* 103 */     body.getBrain().setMemoryWithExpiry(MemoryModuleType.SONIC_BOOM_COOLDOWN, Unit.INSTANCE, cooldown);
/*     */   }
/*     */ }


/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/entity/ai/behavior/warden/SonicBoom.class
