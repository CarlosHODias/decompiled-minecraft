...
/*  39 */     this(speedMultiplier, mob -> DamageTypeTags.PANIC_CAUSES, mob -> LandRandomPos.getPos(mob, 5, 4));
/*     */   }
/*     */   
/*     */   public AnimalPanic(float speedMultiplier, int flyHeight) {
/*  43 */     this(speedMultiplier, mob -> DamageTypeTags.PANIC_CAUSES, mob -> AirAndWaterRandomPos.getPos(mob, 5, 4, flyHeight, (mob.getViewVector(0.0F)).x, (mob.getViewVector(0.0F)).z, 1.5707963705062866D));
/*     */   }
/*     */   
/*     */   public AnimalPanic(float speedMultiplier, Function<PathfinderMob, TagKey<DamageType>> panicCausingDamageTypes) {
/*  47 */     this(speedMultiplier, panicCausingDamageTypes, mob -> LandRandomPos.getPos(mob, 5, 4));
/*     */   }
/*     */   
...
/*  59 */     return ((Boolean)body.getBrain().getMemory(MemoryModuleType.HURT_BY).map(d -> body.is(this.panicCausingDamageTypes.apply(body))).orElse(false) || 
/*  60 */       body.getBrain().hasMemoryValue(MemoryModuleType.IS_PANICKING));
/*     */   }
/*     */ 
/*     */   
...
/*  70 */     body.getBrain().setMemory(MemoryModuleType.IS_PANICKING, true);
...
/*  92 */     if (body.isOnFire()) {
/*  93 */       Optional<Vec3> nearestWater = lookForWater((BlockGetter)level, (Entity)body).map(Vec3::atBottomCenterOf);
/*  94 */       if (nearestWater.isPresent()) {
/*  95 */         return nearestWater.get();
/*     */       }
/*     */     } 
/*     */     
/*  99 */     return this.positionGetter.apply(body);
/*     */   }
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/entity/ai/behavior/AnimalPanic.class
