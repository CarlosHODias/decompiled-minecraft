...
/*     */ 
/*     */     
/*  74 */     NearestVisibleLivingEntities visibleLivingEntities = brain.getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).orElse(NearestVisibleLivingEntities.empty());
/*  75 */     for (LivingEntity entity : (Iterable<LivingEntity>)visibleLivingEntities.findAll(ignored -> true)) {
...
/*  94 */         if (playerNotWearingGold.isEmpty() && !PiglinAi.isWearingSafeArmor((LivingEntity)player) && body.canAttack(entity)) {
/*  95 */           playerNotWearingGold = Optional.of(player);
/*     */         }
/*  97 */         if (playerHoldingWantedItem.isEmpty() && !player.isSpectator() && PiglinAi.isPlayerHoldingLovedItem((LivingEntity)player))
...
/*     */     } 
/*     */     
/* 107 */     List<LivingEntity> livingEntities = (List<LivingEntity>)brain.getMemory(MemoryModuleType.NEAREST_LIVING_ENTITIES).orElse(ImmutableList.of());
...
/* 122 */     brain.setMemory(MemoryModuleType.VISIBLE_ADULT_PIGLIN_COUNT, visibleAdultPiglins.size());
/* 123 */     brain.setMemory(MemoryModuleType.VISIBLE_ADULT_HOGLIN_COUNT, visibleAdultHoglinCount);
/*     */   }
/*     */   
/*     */   private static Optional<BlockPos> findNearestRepellent(ServerLevel level, LivingEntity body) {
/* 127 */     return BlockPos.findClosestMatch(
/* 128 */         body.blockPosition(), 8, 4, pos -> isValidRepellent(level, pos));
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/entity/ai/sensing/PiglinSpecificSensor.class
