...
/*     */   
/*     */   static {
/*  57 */     ATTACK_TARGET_CONDITIONS = TargetingConditions.forCombat().selector((target, level) -> 
/*  58 */         (((Boolean)level.getGameRules().get(GameRules.MOB_GRIEFING) || !target.getType().equals(EntityType.ARMOR_STAND)) && level.getWorldBorder().isWithinBounds(target.getBoundingBox())));
/*     */   }
...
/*  89 */     body.getBrain().setMemory(MemoryModuleType.ATTACK_TARGET_COOLDOWN, TIME_BETWEEN_NON_PLAYER_ATTACKS.sample(random));
/*     */   }
/*     */   
...
/*     */     
/* 101 */     brain.setCoreActivities((Set)ImmutableSet.of(Activity.CORE));
...
/* 108 */     brain.addActivity(Activity.CORE, 0, ImmutableList.of(new AnimalPanic(1.6F), new LookAtTargetSink(45, 90), new MoveToTargetSink(), new CountDownCooldownTicks(MemoryModuleType.TEMPTATION_COOLDOWN_TICKS), new CountDownCooldownTicks(MemoryModuleType.CHARGE_COOLDOWN_TICKS), new CountDownCooldownTicks(MemoryModuleType.ATTACK_TARGET_COOLDOWN)));
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 120 */           Pair.of(1, new AnimalMakeLove(EntityType.NAUTILUS, 0.4F, 2)), 
/* 121 */           Pair.of(2, new FollowTemptation(mob -> 1.3F, mob -> mob.isBaby() ? 2.5D : 3.5D)), 
/* 122 */           Pair.of(3, StartAttacking.create(NautilusAi::findNearestValidAttackTarget)), 
/* 123 */           Pair.of(4, new GateBehavior(
...
/* 131 */                 Pair.of(RandomStroll.swim(1.0F), 2), 
/* 132 */                 Pair.of(SetWalkTargetFromLookTarget.create(1.0F, 3), 3))))));
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 141 */           Pair.of(0, new ChargeAttack(80, ATTACK_TARGET_CONDITIONS, 0.6F, 2.0F, 12.0D, 11.0D, SoundEvents.NAUTILUS_DASH))), 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
...
/* 158 */     if (BehaviorUtils.isBreeding((LivingEntity)body) || !body.isInWater() || body.isBaby() || body.isTame()) {
/* 159 */       return Optional.empty();
/*     */     }
/*     */     
/* 162 */     Optional<LivingEntity> angryAt = BehaviorUtils.getLivingEntityFromUUIDMemory((LivingEntity)body, MemoryModuleType.ANGRY_AT)
/* 163 */       .filter(entity -> (entity.isInWater() && Sensor.isEntityAttackableIgnoringLineOfSight(level, (LivingEntity)body, entity)));
...
/*     */     }
/*     */     
/* 172 */     body.getBrain().setMemory(MemoryModuleType.ATTACK_TARGET_COOLDOWN, TIME_BETWEEN_NON_PLAYER_ATTACKS.sample(level.random));
/*     */     
/* 174 */     if (level.random.nextFloat() < 0.5F) {
/* 175 */       return Optional.empty();
/*     */     }
/*     */     
/* 178 */     Optional<LivingEntity> target = ((NearestVisibleLivingEntities)
/* 179 */       body.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).orElse(NearestVisibleLivingEntities.empty()))
/* 180 */       .findClosest(NautilusAi::isHostileTarget);
/*     */     
/* 182 */     return target;
/*     */   }
/*     */   
/*     */   protected static void setAngerTarget(ServerLevel level, AbstractNautilus body, LivingEntity target) {
/* 186 */     if (Sensor.isEntityAttackableIgnoringLineOfSight(level, (LivingEntity)body, target)) {
...
/* 193 */     return (mob.isInWater() && mob.getType().is(EntityTypeTags.NAUTILUS_HOSTILES));
/*     */   }
/*     */   
/*     */   public static void updateActivity(Nautilus body) {
/* 197 */     body.getBrain().setActiveActivityToFirstValid((List)ImmutableList.of(Activity.FIGHT, Activity.IDLE));
/*     */   }
/*     */   
/*     */   public static Predicate<ItemStack> getTemptations() {
/* 201 */     return i -> i.is(ItemTags.NAUTILUS_FOOD);
/*     */   }
/*     */ }


/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/entity/animal/nautilus/NautilusAi.class
