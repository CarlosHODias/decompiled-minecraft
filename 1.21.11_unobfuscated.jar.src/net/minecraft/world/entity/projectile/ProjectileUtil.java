...
/*  77 */     BlockHitResult blockHitResult = level.clipIncludingBorder(new ClipContext(from, to, clipType, ClipContext.Fluid.NONE, source));
/*     */     
/*  79 */     if (blockHitResult.getType() != HitResult.Type.MISS) {
/*  80 */       to = blockHitResult.getLocation();
/*     */     }
...
/*     */     
/* 121 */     for (Entity entity : (Iterable<Entity>)level.getEntities(except, box, matching)) {
...
/* 125 */         if (nearest >= 0.0D) {
/* 126 */           hovered = entity;
/* 127 */           hoveredPos = clipPoint.orElse(from);
/* 128 */           nearest = 0.0D;
/*     */         }  continue;
/*     */       } 
/* 131 */       if (clipPoint.isPresent()) {
/* 132 */         Vec3 location = clipPoint.get();
...
/* 157 */     return getEntityHitResult(level, source, from, to, targetSearchArea, matching, computeMargin(source));
/*     */   }
/*     */   
/*     */   public static float computeMargin(Entity source) {
/* 161 */     return Math.max(0.0F, Math.min(0.3F, (source.tickCount - 2) / 20.0F));
/*     */   }
/*     */   
...
/*     */     
/* 169 */     for (Entity entity : (Iterable<Entity>)level.getEntities(source, targetSearchArea, matching)) {
...
/* 173 */         double dd = from.distanceToSqr(location.get());
/* 174 */         if (dd < nearest) {
...
/* 183 */       return null;
/*     */     }
/* 185 */     return new EntityHitResult(hitEntity, nearestLocation.get());
/*     */   }
/*     */   
...
/* 189 */     return getManyEntityHitResult(level, source, from, to, targetSearchArea, matching, computeMargin(source), ClipContext.Block.COLLIDER, includeFromEntity);
/*     */   }
/*     */   
...
/* 193 */     List<EntityHitResult> collector = new ArrayList<>();
/*     */     
/* 195 */     for (Entity entity : (Iterable<Entity>)level.getEntities(source, targetSearchArea, matching)) {
...
/* 205 */         collector.add(new EntityHitResult(entity, exactHit.get()));
/*     */         continue;
/*     */       } 
...
/*     */         continue;
/*     */       }
/* 215 */       Vec3 outsideHitPosition = outsideHit.get();
...
/* 224 */         collector.add(new EntityHitResult(entity, surfaceHit.get()));
/*     */       }
/*     */     } 
/*     */     
...
/* 264 */     return mob.getMainHandItem().is(weaponItem) ? InteractionHand.MAIN_HAND : InteractionHand.OFF_HAND;
/*     */   }
/*     */   
/*     */   public static AbstractArrow getMobArrow(LivingEntity mob, ItemStack projectile, float power, ItemStack firedFromWeapon) {
/* 268 */     ArrowItem arrowItem = (projectile.getItem() instanceof ArrowItem) ? (ArrowItem)projectile.getItem() : (ArrowItem)Items.ARROW;
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/entity/projectile/ProjectileUtil.class
