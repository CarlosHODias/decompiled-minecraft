...
/*  50 */     super((EntityType)type, level);
/*     */     
...
/*  93 */     this.currentMoveDirection = input.read("Dir", Direction.LEGACY_ID_CODEC).orElse(null);
...
/* 106 */     this.currentMoveDirection = direction;
/*     */   }
/*     */   
...
/* 124 */     if (!targetPos.closerToCenterThan((Position)position(), 2.0D)) {
...
/*     */         } 
/*     */       } else {
/* 158 */         selection = options.get(this.random.nextInt(options.size()));
/*     */       } 
/*     */       
...
/*     */         
/* 215 */         Vec3 vec3 = getDeltaMovement();
/* 216 */         setDeltaMovement(vec3.add((this.targetDeltaX - vec3.x) * 0.2D, (this.targetDeltaY - vec3.y) * 0.2D, (this.targetDeltaZ - vec3.z) * 0.2D));
/*     */       
/*     */       }
/*     */       else {
/*     */ 
...
/* 242 */       level().addParticle((ParticleOptions)ParticleTypes.END_ROD, getX() - movement.x, getY() - movement.y + 0.15D, getZ() - movement.z, 0.0D, 0.0D, 0.0D);
...
/* 259 */           if ((axis == Direction.Axis.X && current.getX() == targetPos.getX()) || (axis == Direction.Axis.Z && 
/* 260 */             current.getZ() == targetPos.getZ()) || (axis == Direction.Axis.Y && 
/* 261 */             current.getY() == targetPos.getY()))
/*     */           {
...
/* 278 */     return (super.canHitEntity(entity) && !entity.noPhysics);
/*     */   }
/*     */ 
/*     */   
...
/* 345 */     level.sendParticles((ParticleOptions)ParticleTypes.CRIT, getX(), getY(), getZ(), 15, 0.2D, 0.2D, 0.2D, 0.0D);
/* 346 */     destroy();
/* 347 */     return true;
/*     */   }
/*     */ 
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/entity/projectile/ShulkerBullet.class
