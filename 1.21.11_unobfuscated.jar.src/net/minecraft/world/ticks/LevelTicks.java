...
/*     */   
/*     */   static {
/*  32 */     CONTAINER_DRAIN_ORDER = ((o1, o2) -> ScheduledTick.INTRA_TICK_DRAIN_ORDER.compare(o1.peek(), o2.peek()));
/*     */   }
/*     */ 
/*     */   
/*  36 */   private final Long2ObjectMap<LevelChunkTicks<T>> allContainers = (Long2ObjectMap<LevelChunkTicks<T>>)new it.unimi.dsi.fastutil.longs.Long2ObjectOpenHashMap(); private final Long2LongMap nextTickForContainer; private final Queue<LevelChunkTicks<T>> containersToTick; private final Queue<ScheduledTick<T>> toRunThisTick; public LevelTicks(LongPredicate tickCheck) {
/*  37 */     this.nextTickForContainer = (Long2LongMap)Util.make(new Long2LongOpenHashMap(), m -> m.defaultReturnValue(Long.MAX_VALUE));
/*     */     
/*  39 */     this.containersToTick = (Queue)new java.util.PriorityQueue<>(CONTAINER_DRAIN_ORDER);
/*  40 */     this.toRunThisTick = new ArrayDeque<>();
/*  41 */     this.alreadyRunThisTick = new ArrayList<>();
/*     */ 
/*     */     
/*  44 */     this.toRunThisTickSet = (Set<ScheduledTick<?>>)new it.unimi.dsi.fastutil.objects.ObjectOpenCustomHashSet(ScheduledTick.UNIQUE_TICK_HASH);
/*     */     
/*  46 */     this.chunkScheduleUpdater = ((container, newTick) -> {
/*     */         if (newTick.equals(container.peek())) {
/*     */           updateContainerScheduling(newTick);
/*     */         }
/*     */       });
/*     */ 
/*     */ 
...
/*  72 */     LevelChunkTicks<T> removedContainer = (LevelChunkTicks<T>)this.allContainers.remove(chunkKey);
...
/*  82 */     LevelChunkTicks<T> tickContainer = (LevelChunkTicks<T>)this.allContainers.get(chunkKey);
...
/* 116 */         LevelChunkTicks<T> candidateContainer = (LevelChunkTicks<T>)this.allContainers.get(chunkPos);
...
/* 141 */     while (canScheduleMoreTicks(maxTicksToProcess) && (topContainer = this.containersToTick.poll()) != null) {
...
/* 170 */     this.nextTickForContainer.put(ChunkPos.asLong(nextTick.pos()), nextTick.triggerTick());
/*     */   }
/*     */   
...
/*     */     }
/*     */     
/* 178 */     LevelChunkTicks<T> nextBestContainer = containersToTick.peek();
...
/* 197 */     this.toRunThisTick.add(tick);
/*     */   }
/*     */   
/*     */   private boolean canScheduleMoreTicks(int maxTicksToProcess) {
/* 201 */     return (this.toRunThisTick.size() < maxTicksToProcess);
/*     */   }
/*     */   
...
/*     */ 
/*     */       
/* 208 */       ScheduledTick<T> entry = this.toRunThisTick.poll();
...
/* 228 */     LevelChunkTicks<T> tickContainer = (LevelChunkTicks<T>)this.allContainers.get(ChunkPos.asLong(pos));
...
/* 260 */         LevelChunkTicks<T> container = (LevelChunkTicks<T>)this.allContainers.get(containerPos);
...
/*     */     Predicate<ScheduledTick<T>> tickInsideBB = t -> area.isInside((Vec3i)t.pos());
...
/*     */             if (newTop != null) {
/*     */               updateContainerScheduling(newTop);
/*     */             } else {
/*     */               this.nextTickForContainer.remove(tickInsideBB);
/*     */             } 
/*     */           }
/*     */         });
...
/* 295 */     copyAreaFrom(this, area, offset);
/*     */   }
/*     */   
/*     */   public void copyAreaFrom(LevelTicks<T> source, BoundingBox area, Vec3i offset) {
/* 299 */     List<ScheduledTick<T>> ticksToAdd = new ArrayList<>();
/*     */     
/*     */     Predicate<ScheduledTick<T>> tickInsideBB = t -> area.isInside((Vec3i)t.pos());
/*     */     
...
/*     */     
/* 306 */     source.forContainersInArea(area, (pos, container) -> {
/*     */           Objects.requireNonNull(ticksToAdd); container.getAll().filter(tickInsideBB).forEach(ticksToAdd::add);
...
/*     */     
/* 312 */     ticksToAdd.forEach(tick -> schedule(new ScheduledTick<>(offset.type(), offset.pos().offset(offset), offset.triggerTick(), offset.priority(), offset.subTickOrder() - offset + minSubTick + 1L)));
/*     */   }
/*     */ 
/*     */   
/*     */   public int count() {
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/ticks/LevelTicks.class
