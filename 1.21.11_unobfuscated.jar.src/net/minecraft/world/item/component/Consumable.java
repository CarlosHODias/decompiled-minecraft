...
/*  34 */     //   0	7	0	this	Lnet/minecraft/world/item/component/Consumable; } public float consumeSeconds() { return this.consumeSeconds; } public final int hashCode() { // Byte code:
/*     */     //   0: aload_0
...
/*  34 */     //   0	8	1	o	Ljava/lang/Object; } public ItemUseAnimation animation() { return this.animation; } public Holder<SoundEvent> sound() { return this.sound; } public boolean hasConsumeParticles() { return this.hasConsumeParticles; } public List<ConsumeEffect> onConsumeEffects() { return this.onConsumeEffects; }
/*     */ 
/*     */ 
/*     */ 
/*     */ 
...
/*     */   
/*     */   static {
/*  46 */     CODEC = RecordCodecBuilder.create(i -> i.group((App)net.minecraft.util.ExtraCodecs.NON_NEGATIVE_FLOAT.optionalFieldOf("consume_seconds", 1.6F).forGetter(Consumable::consumeSeconds), (App)ItemUseAnimation.CODEC.optionalFieldOf("animation", ItemUseAnimation.EAT).forGetter(Consumable::animation), (App)SoundEvent.CODEC.optionalFieldOf("sound", SoundEvents.GENERIC_EAT).forGetter(Consumable::sound), (App)Codec.BOOL.optionalFieldOf("has_consume_particles", true).forGetter(Consumable::hasConsumeParticles), (App)ConsumeEffect.CODEC.listOf().optionalFieldOf("on_consume_effects", List.of()).forGetter(Consumable::onConsumeEffects)).apply((com.mojang.datafixers.kinds.Applicative)i, Consumable::new));
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/*     */ 
/*     */   
/*  54 */   public static final net.minecraft.network.codec.StreamCodec<net.minecraft.network.RegistryFriendlyByteBuf, Consumable> STREAM_CODEC = net.minecraft.network.codec.StreamCodec.composite(ByteBufCodecs.FLOAT, Consumable::consumeSeconds, ItemUseAnimation.STREAM_CODEC, Consumable::animation, SoundEvent.STREAM_CODEC, Consumable::sound, ByteBufCodecs.BOOL, Consumable::hasConsumeParticles, 
/*     */ 
/*     */ 
/*     */ 
/*     */       
/*  59 */       ConsumeEffect.STREAM_CODEC.apply(ByteBufCodecs.list()), Consumable::onConsumeEffects, Consumable::new);
/*     */ 
/*     */ 
/*     */   
/*     */   public InteractionResult startConsuming(LivingEntity user, ItemStack stack, net.minecraft.world.InteractionHand hand) {
/*  64 */     if (!canConsume(user, stack)) {
/*  65 */       return (InteractionResult)InteractionResult.FAIL;
/*     */     }
/*     */     
/*  68 */     boolean consumesOverTime = (consumeTicks() > 0);
/*  69 */     if (consumesOverTime) {
/*  70 */       user.startUsingItem(hand);
/*  71 */       return (InteractionResult)InteractionResult.CONSUME;
/*     */     } 
/*     */     
/*  74 */     ItemStack result = onConsume(user.level(), user, stack);
/*  75 */     return (InteractionResult)InteractionResult.CONSUME.heldItemTransformedTo(result);
/*     */   }
/*     */   
...
/*  83 */       serverPlayer.awardStat(net.minecraft.stats.Stats.ITEM_USED.get(stack.getItem()));
/*  84 */       net.minecraft.advancements.CriteriaTriggers.CONSUME_ITEM.trigger(serverPlayer, stack); }
/*     */ 
/*     */     
/*  87 */     stack.getAllOfType(ConsumableListener.class).forEach(component -> stack.onConsume(level, level, user, this));
/*  88 */     if (!level.isClientSide()) {
/*  89 */       this.onConsumeEffects.forEach(action -> action.apply(level, stack, user));
/*     */     }
/*     */     
/*  92 */     user.gameEvent((this.animation == ItemUseAnimation.DRINK) ? (Holder)net.minecraft.world.level.gameevent.GameEvent.DRINK : (Holder)net.minecraft.world.level.gameevent.GameEvent.EAT);
/*  93 */     stack.consume(1, user);
/*  94 */     return stack;
/*     */   }
/*     */   
/*     */   public boolean canConsume(LivingEntity user, ItemStack stack) {
/*  98 */     FoodProperties foodProperties = (FoodProperties)stack.get(net.minecraft.core.component.DataComponents.FOOD);
...
/* 106 */     return (int)(this.consumeSeconds * 20.0F);
/*     */   }
/*     */   
...
/* 113 */     float drinkPitch = net.minecraft.util.Mth.randomBetween(random, 0.9F, 1.0F);
/*     */     
...
/* 141 */     private final List<ConsumeEffect> onConsumeEffects = new java.util.ArrayList<>();
/*     */ 
/*     */ 
/*     */     
...
/* 161 */       return onConsume((ConsumeEffect)new net.minecraft.world.item.consume_effects.PlaySoundConsumeEffect(soundAfterConsume));
/*     */     }
/*     */     
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/item/component/Consumable.class
