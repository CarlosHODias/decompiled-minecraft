...
/*     */   }
/*     */   
/*     */   public static MapItemSavedData getSavedData(MapId id, Level level) {
/*  51 */     return (id == null) ? null : level.getMapData(id);
/*     */   }
/*     */   
...
/*     */                 
/* 124 */                 if ((ceilingNoise >> 20 & 0x1) == 0) {
/* 125 */                   linkedHashMultiset.add(Blocks.DIRT.defaultBlockState().getMapColor((BlockGetter)level, BlockPos.ZERO), 10);
/*     */                 } else {
/* 127 */                   linkedHashMultiset.add(Blocks.STONE.defaultBlockState().getMapColor((BlockGetter)level, BlockPos.ZERO), 100);
/*     */                 } 
/*     */                 
...
/* 145 */                         state = chunk.getBlockState((BlockPos)blockPos);
/* 146 */                       } while (state.getMapColor((BlockGetter)level, (BlockPos)blockPos) == MapColor.NONE && columnY > level.getMinY());
/*     */                       
/* 148 */                       if (columnY > level.getMinY() && !state.getFluidState().isEmpty()) {
/*     */                         BlockState belowBlock;
/* 150 */                         int solidY = columnY - 1;
/*     */                         
/* 152 */                         belowPos.set((Vec3i)blockPos);
/*     */                         do {
/* 154 */                           belowPos.setY(solidY--);
/* 155 */                           belowBlock = chunk.getBlockState((BlockPos)belowPos);
/* 156 */                           waterDepth++;
/* 157 */                         } while (solidY > level.getMinY() && !belowBlock.getFluidState().isEmpty());
/*     */                         
/* 159 */                         state = getCorrectStateForFluidBlock(level, state, (BlockPos)blockPos);
/*     */                       } 
/*     */                     } else {
/* 162 */                       state = Blocks.BEDROCK.defaultBlockState();
/*     */                     } 
/*     */                     
/* 165 */                     data.checkBanners((BlockGetter)level, blockPos.getX(), blockPos.getZ());
/*     */                     
/* 167 */                     averageAreaHeight += columnY / (scale * scale);
/*     */                     
/* 169 */                     linkedHashMultiset.add(state.getMapColor((BlockGetter)level, (BlockPos)blockPos));
/*     */                   } 
/*     */                 } 
/*     */               } 
/* 173 */               waterDepth /= scale * scale;
/*     */               
/* 175 */               MapColor color = (MapColor)Iterables.getFirst((Iterable)Multisets.copyHighestCountFirst((Multiset)linkedHashMultiset), MapColor.NONE);
/*     */               
/* 177 */               if (color == MapColor.WATER) {
/* 178 */                 double diff = waterDepth * 0.1D + (imgX + imgY & 0x1) * 0.2D;
/* 179 */                 if (diff < 0.5D) {
...
/*     */                 } 
/*     */               } else {
/* 187 */                 double diff = (averageAreaHeight - previousAverageAreaHeight) * 4.0D / (scale + 4) + ((imgX + imgY & 0x1) - 0.5D) * 0.4D;
/* 188 */                 if (diff > 0.6D) {
...
/*     */                 {
/*     */                   
/* 205 */                   if (!ditherBlack || (imgX + imgY & 0x1) != 0)
/*     */                   {
/*     */ 
/*     */                     
...
/* 216 */     if (!fluidState.isEmpty() && !state.isFaceSturdy((BlockGetter)level, pos, net.minecraft.core.Direction.UP)) {
...
/* 224 */     return isBiomeWatery[z * 128 + x];
/*     */   }
/*     */   
/*     */   public static void renderBiomePreviewMap(ServerLevel level, ItemStack mapItemStack) {
/* 228 */     MapItemSavedData data = getSavedData(mapItemStack, (Level)level);
/* 229 */     if (data == null) {
/*     */       return;
/*     */     }
...
/* 248 */         Holder<Biome> biome = level.getBiome((BlockPos)pos.set((unscaledStartX + column) * scale, 0, (unscaledStartZ + row) * scale));
...
/* 301 */     MapItemSavedData data = getSavedData(itemStack, (Level)level);
/* 302 */     if (data == null) {
/*     */       return;
/*     */     }
/*     */     
...
/* 311 */       update((Level)level, owner, data);
/*     */     }
/*     */   }
/*     */ 
/*     */ 
...
/* 319 */     MapPostProcessing postProcessing = itemStack.<MapPostProcessing>remove(DataComponents.MAP_POST_PROCESSING);
...
/* 332 */     MapItemSavedData original = getSavedData(itemStack, (Level)level);
/*     */     
...
/* 342 */     MapItemSavedData mapData = getSavedData(map, (Level)level);
/* 343 */     if (mapData != null) {
...
/* 357 */         if (data != null && 
/* 358 */           !data.toggleBanner((LevelAccessor)context.getLevel(), context.getClickedPos())) {
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/item/MapItem.class
