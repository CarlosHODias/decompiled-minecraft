...
/* 37 */       AreaEffectCloud cloud = clouds.get(0);
...
/* 41 */       level.gameEvent((Entity)player, (Holder)GameEvent.FLUID_PICKUP, player.position());
/* 42 */       if (player instanceof ServerPlayer) { ServerPlayer serverPlayer = (ServerPlayer)player;
/* 43 */         CriteriaTriggers.PLAYER_INTERACTED_WITH_ENTITY.trigger(serverPlayer, itemStack, (Entity)cloud); }
/*    */ 
/*    */       
/* 46 */       return (InteractionResult)InteractionResult.SUCCESS.heldItemTransformedTo(turnBottleIntoItem(itemStack, player, new ItemStack(Items.DRAGON_BREATH)));
/*    */     } 
/*    */     
...
/*    */       
/* 57 */       if (!level.mayInteract((Entity)player, pos)) {
/* 58 */         return (InteractionResult)InteractionResult.PASS;
/*    */       }
/* 60 */       if (level.getFluidState(pos).is(FluidTags.WATER)) {
/* 61 */         level.playSound((Entity)player, player.getX(), player.getY(), player.getZ(), SoundEvents.BOTTLE_FILL, SoundSource.NEUTRAL, 1.0F, 1.0F);
/* 62 */         level.gameEvent((Entity)player, (Holder)GameEvent.FLUID_PICKUP, pos);
/* 63 */         return (InteractionResult)InteractionResult.SUCCESS.heldItemTransformedTo(turnBottleIntoItem(itemStack, player, PotionContents.createItemStack(Items.POTION, Potions.WATER)));
/*    */       } 
/*    */     } 
/*    */     
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/item/BottleItem.class
