...
/* 43 */     List<Entity> entities = level.getEntities((Entity)player, player.getBoundingBox().expandTowards(viewVector.scale(5.0D)).inflate(1.0D), EntitySelector.CAN_BE_PICKED);
...
/* 55 */       AbstractBoat boat = getBoat(level, (HitResult)blockHitResult, itemStack, player);
/* 56 */       if (boat == null) {
/* 57 */         return (InteractionResult)InteractionResult.FAIL;
/*    */       }
/* 59 */       boat.setYRot(player.getYRot());
/* 60 */       if (!level.noCollision((Entity)boat, boat.getBoundingBox())) {
/* 61 */         return (InteractionResult)InteractionResult.FAIL;
/*    */       }
/* 63 */       if (!level.isClientSide()) {
/* 64 */         level.addFreshEntity((Entity)boat);
/* 65 */         level.gameEvent((Entity)player, (Holder)GameEvent.ENTITY_PLACE, blockHitResult.getLocation());
/* 66 */         itemStack.consume(1, (LivingEntity)player);
/*    */       } 
...
/* 83 */         EntityType.createDefaultStackConfig((Level)serverLevel, itemStack, (LivingEntity)player).accept(boat); }
/*    */     
/*    */     } 
/* 86 */     return boat;
/*    */   }
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/item/BoatItem.class
