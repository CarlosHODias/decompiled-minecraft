...
/*  56 */     List<Holder<StructureSet>> structures = structureOverrides.filter(structureSet -> hasBiomesForStructureSet((StructureSet)structureSet.value(), biomeSource))
/*  57 */       .toList();
/*     */     
...
/*  63 */     List<Holder<StructureSet>> structures = (List<Holder<StructureSet>>)
/*     */       
/*  65 */       allStructures.listElements()
/*  66 */       .filter(structureSet -> hasBiomesForStructureSet((StructureSet)structureSet.value(), biomeSource))
/*  67 */       .collect(Collectors.toUnmodifiableList());
/*     */     
...
/*  73 */     Stream<Holder<Biome>> structureBiomes = structureSet.structures().stream()
/*  74 */       .flatMap(entry -> {
...
/*     */           StructureSet set = (StructureSet)possibleBiomes.value();
/*     */           boolean hasAnyPlaceableStructures = false;
/*     */           for (StructureSet.StructureSelectionEntry entry : (Iterable<StructureSet.StructureSelectionEntry>)set.structures()) {
/*     */             Structure structure = (Structure)entry.structure().value();
/*     */             Objects.requireNonNull(possibleBiomes);
/*     */             if (structure.biomes().stream().anyMatch(possibleBiomes::contains)) {
/*     */               ((List<StructurePlacement>)this.placementsForStructure.computeIfAbsent(structure, ())).add(set.placement());
...
/*     */               this.ringPositions.put(ringsPlacement, generateRingPositions(possibleBiomes, ringsPlacement));
/*     */             } 
/*     */           } 
...
/*     */     
/* 124 */     List<CompletableFuture<ChunkPos>> tasks = new ArrayList<>(count);
/*     */     
...
/* 180 */     CompletableFuture<List<ChunkPos>> result = this.ringPositions.get(placement);
/* 181 */     return (result != null) ? result.join() : null;
/*     */   }
/*     */   
/*     */   public List<StructurePlacement> getPlacementsForStructure(Holder<Structure> structure) {
/* 185 */     ensureStructuresGenerated();
/* 186 */     return this.placementsForStructure.getOrDefault(structure.value(), List.of());
/*     */   }
/*     */   
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/level/chunk/ChunkGeneratorStructureState.class
