...
/*     */     }
/*  46 */     return new StructureManager((LevelAccessor)region, this.worldOptions, this.structureCheck);
/*     */   }
/*     */   
...
/*  54 */       Structure structure = entry.getKey();
/*  55 */       if (matcher.test(structure)) {
/*  56 */         Objects.requireNonNull(result); fillStartsForStructure(structure, entry.getValue(), result::add);
/*     */       } 
/*     */     } 
/*     */     
/*  60 */     return (List<StructureStart>)result.build();
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/*  71 */     return (List<StructureStart>)result.build();
/*     */   }
/*     */   
/*     */   public void fillStartsForStructure(Structure structure, LongSet referencesForStructure, Consumer<StructureStart> consumer) {
/*  75 */     for (LongIterator<Long> longIterator = referencesForStructure.iterator(); longIterator.hasNext(); ) { long key = (Long)longIterator.next();
/*  76 */       SectionPos sectionPos = SectionPos.of(new ChunkPos(key), this.level.getMinSectionY());
/*  77 */       StructureStart start = getStartForStructure(sectionPos, structure, (StructureAccess)this.level.getChunk(sectionPos.x(), sectionPos.z(), ChunkStatus.STRUCTURE_STARTS));
...
/*     */   }
/*     */   
/*     */   public StructureStart getStartForStructure(SectionPos pos, Structure structure, StructureAccess chunk) {
/*  85 */     return chunk.getStartForStructure(structure);
/*     */   }
/*     */   
/*     */   public void setStartForStructure(SectionPos pos, Structure structure, StructureStart start, StructureAccess chunk) {
/*  89 */     chunk.setStartForStructure(structure, start);
/*     */   }
/*     */   
/*     */   public void addReferenceForStructure(SectionPos pos, Structure structure, long reference, StructureAccess chunk) {
/*  93 */     chunk.addReferenceForStructure(structure, reference);
/*     */   }
/*     */   
...
/* 105 */       if (structureStart.getBoundingBox().isInside((Vec3i)blockPos)) {
...
/* 113 */     return getStructureWithPieceAt(blockPos, structure -> structure.is(structureTag));
/*     */   }
/*     */   
/*     */   public StructureStart getStructureWithPieceAt(BlockPos blockPos, HolderSet<Structure> structures) {
/* 117 */     Objects.requireNonNull(structures); return getStructureWithPieceAt(blockPos, structures::contains);
/*     */   }
/*     */ 
/*     */   
...
/* 123 */     for (StructureStart structureStart : startsForStructure(new ChunkPos(blockPos), s -> { Objects.requireNonNull(predicate); return (Boolean)structures.get(structures.getId(s)).map(predicate::test).orElse(false);
...
/* 144 */     for (StructurePiece piece : (Iterable<StructurePiece>)structureStart.getPieces()) {
/* 145 */       if (piece.getBoundingBox().isInside((Vec3i)blockPos)) {
/* 146 */         return true;
/*     */       }
/*     */     } 
/* 149 */     return false;
...
/* 163 */     return this.structureCheck.checkStart(pos, structure, placement, createReference);
/*     */   }
/*     */   
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/level/StructureManager.class
