...
/*  59 */     super(BlockEntityType.CONDUIT, worldPosition, blockState);
/*     */   }
/*     */ 
/*     */   
...
/*  81 */     return saveCustomOnly(registries);
/*     */   }
/*     */   
...
/*  96 */     animationTick(level, pos, effectBlocks, (Entity)destroyTarget, entity.tickCount);
...
/* 136 */     entity.setHunting((effectBlocks.size() >= 42));
/*     */   }
/*     */   
...
/*     */ 
/*     */     
/* 155 */     for (int i = -2; i <= 2; i++) {
/* 156 */       for (int oy = -2; oy <= 2; oy++) {
/* 157 */         for (int oz = -2; oz <= 2; oz++) {
/* 158 */           int ax = Math.abs(i);
/* 159 */           int ay = Math.abs(oy);
/* 160 */           int az = Math.abs(oz);
/* 161 */           if (ax > 1 || ay > 1 || az > 1)
/*     */           {
/*     */             
/* 164 */             if ((i == 0 && (ay == 2 || az == 2)) || (oy == 0 && (ax == 2 || az == 2)) || (oz == 0 && (ax == 2 || ay == 2))) {
/* 165 */               BlockPos testPos = worldPosition.offset(i, oy, oz);
...
/* 196 */       if (worldPosition.closerThan((Vec3i)player.blockPosition(), effectRange) && player.isInWaterOrRain()) {
...
/*     */     
/* 205 */     LivingEntity targetEntity = EntityReference.getLivingEntity(newDestroyTarget, (Level)level);
...
/*     */     }
/* 224 */     LivingEntity targetEntity = EntityReference.getLivingEntity(target, (Level)level);
/* 225 */     if (targetEntity == null || !targetEntity.isAlive() || !pos.closerThan((Vec3i)targetEntity.blockPosition(), 8.0D))
/*     */     {
/* 227 */       return null;
/*     */     }
/* 229 */     return target;
...
/* 235 */       return null;
/*     */     }
/* 237 */     return EntityReference.of((UniquelyIdentifyable)Util.getRandom(candidates, level.random));
/*     */   }
/*     */   
/*     */   private static AABB getDestroyRangeAABB(BlockPos worldPosition) {
/* 241 */     return new AABB(worldPosition).inflate(8.0D);
/*     */   }
/*     */   
...
/*     */       }
/*     */       
/* 256 */       BlockPos delta = pos.subtract((Vec3i)worldPosition);
...
/* 260 */       level.addParticle((ParticleOptions)ParticleTypes.NAUTILUS, particleEnd.x, particleEnd.y, particleEnd.z, dx, dy, dz);
/*     */     } 
/*     */     
...
/* 269 */       level.addParticle((ParticleOptions)ParticleTypes.NAUTILUS, targetPosition.x, targetPosition.y, targetPosition.z, velocity.x, velocity.y, velocity.z);
/*     */     } 
/*     */   }
/*     */   
...
/* 282 */     this.isHunting = hunting;
/*     */   }
/*     */   
/*     */   public float getActiveRotation(float a) {
/* 286 */     return (this.activeRotation + a) * -0.0375F;
/*     */   }
/*     */ }


/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/level/block/entity/ConduitBlockEntity.class
