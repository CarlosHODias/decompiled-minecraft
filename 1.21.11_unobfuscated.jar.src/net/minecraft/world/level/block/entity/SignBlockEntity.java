...
/*  50 */     this(BlockEntityType.SIGN, worldPosition, blockState);
/*     */   }
/*     */   
...
/*  77 */     return isFrontText ? this.frontText : this.backText;
/*     */   }
/*     */   
...
/*     */     
/* 109 */     this.frontText = input.read("front_text", SignText.DIRECT_CODEC).map(this::loadLines).orElseGet(SignText::new);
/* 110 */     this.backText = input.read("back_text", SignText.DIRECT_CODEC).map(this::loadLines).orElseGet(SignText::new);
...
/*     */       try {
/* 126 */         return (Component)ComponentUtils.updateForEntity(createCommandSourceStack(null, serverLevel, this.worldPosition), component, null, 0);
...
/*     */       return;
/*     */     } 
/* 140 */     updateText(text -> setMessages(player, player, lines), frontText);
...
/* 147 */     return setText(function.apply(text), isFrontText);
/*     */   }
/*     */   
/*     */   private SignText setMessages(Player player, List<FilteredText> lines, SignText text) {
/* 151 */     for (int i = 0; i < lines.size(); i++) {
/* 152 */       FilteredText line = lines.get(i);
/* 153 */       Style currentTextStyle = text.getMessage(i, player.isTextFilteringEnabled()).getStyle();
/* 154 */       if (player.isTextFilteringEnabled()) {
/*     */         
/* 156 */         text = text.setMessage(i, (Component)Component.literal(line.filteredOrEmpty()).setStyle(currentTextStyle));
/*     */       } else {
/* 158 */         text = text.setMessage(i, (Component)Component.literal(line.raw()).setStyle(currentTextStyle), (Component)Component.literal(line.filteredOrEmpty()).setStyle(currentTextStyle));
/*     */       } 
/*     */     } 
/* 161 */     return text;
/*     */   }
...
/* 165 */     return isFrontText ? setFrontText(text) : setBackText(text);
/*     */   }
/*     */   
...
/* 187 */     return (isWaxed() && getText(isFrontText).hasAnyClickCommands(player));
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 217 */     Component displayName = (player == null) ? (Component)Component.literal("Sign") : player.getDisplayName();
/* 218 */     return new CommandSourceStack(CommandSource.NULL, Vec3.atCenterOf((Vec3i)pos), Vec2.ZERO, level, (PermissionSet)LevelBasedPermissionSet.GAMEMASTER, textName, displayName, level.getServer(), (Entity)player);
/*     */   }
/*     */ 
/*     */   
...
/* 228 */     return saveCustomOnly(registries);
/*     */   }
/*     */   
/*     */   public void setAllowedPlayerEditor(UUID playerUUID) {
/* 232 */     this.playerWhoMayEdit = playerUUID;
/*     */   }
/*     */   
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/level/block/entity/SignBlockEntity.class
