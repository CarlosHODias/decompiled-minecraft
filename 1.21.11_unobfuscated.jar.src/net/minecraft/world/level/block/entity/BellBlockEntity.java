...
/*  43 */     super(BlockEntityType.BELL, worldPosition, blockState);
/*     */   }
/*     */ 
/*     */   
...
/*  90 */     tick(level, pos, state, entity, BellBlockEntity::showBellParticles);
/*     */   }
/*     */   
/*     */   public static void serverTick(Level level, BlockPos pos, BlockState state, BellBlockEntity entity) {
/*  94 */     tick(level, pos, state, entity, BellBlockEntity::makeRaidersGlow);
/*     */   }
/*     */   
...
/*     */           continue;
/*     */         }
/* 124 */         if (blockPos.closerToCenterThan((Position)entity.position(), 32.0D)) {
/* 125 */           entity.getBrain().setMemory(MemoryModuleType.HEARD_BELL_TIME, this.level.getGameTime());
/*     */         }
/*     */       } 
/*     */     }
/*     */   }
...
/*     */         continue;
/*     */       }
/* 136 */       if (bellPos.closerToCenterThan((Position)entity.position(), 32.0D) && 
...
/*     */     
/* 154 */     int nearbyRaiderCount = (int)nearbyEntities.stream().filter(p -> bellPos.closerToCenterThan((Position)p.position(), 48.0D)).count();
/*     */     
/* 156 */     nearbyEntities.stream()
/* 157 */       .filter(e -> isRaiderWithinRange(bellPos, e))
/* 158 */       .forEach(entity -> {
/*     */           float distAway = 1.0F;
/*     */           double distBtwn = Math.sqrt((entity.getX() - bellPos.getX()) * (entity.getX() - bellPos.getX()) + (entity.getZ() - bellPos.getZ()) * (entity.getZ() - bellPos.getZ())), x3 = (bellPos.getX() + 0.5F) + 1.0D / distBtwn * (entity.getX() - bellPos.getX()), z3 = (bellPos.getZ() + 0.5F) + 1.0D / distBtwn * (entity.getZ() - bellPos.getZ());
/*     */           int particleCount = Mth.clamp((nearbyRaiderCount - 21) / -2, 3, 15);
/*     */           for (int i = 0; i < particleCount; i++) {
/*     */             int color = particleColor.addAndGet(5);
/*     */             level.addParticle((ParticleOptions)ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, color), x3, (bellPos.getY() + 0.5F), z3, 0.0D, 0.0D, 0.0D);
/*     */           } 
/*     */         });
/*     */   }
/*     */ 
...
/* 174 */       !entity.isRemoved() && 
/* 175 */       blockPos.closerToCenterThan((Position)entity.position(), 48.0D) && 
/* 176 */       entity.getType().is(EntityTypeTags.RAIDERS));
/*     */   }
/*     */   
/*     */   private static void glow(LivingEntity raider) {
/* 180 */     raider.addEffect(new MobEffectInstance(MobEffects.GLOWING, 60));
/*     */   }
/*     */   
/*     */   @FunctionalInterface
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/level/block/entity/BellBlockEntity.class
