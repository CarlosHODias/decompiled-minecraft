...
/*  51 */     this(name, 0.0F, megaTree, Optional.empty(), tree, Optional.empty(), flowers, Optional.empty());
/*     */   }
/*     */   
...
/*  92 */       Holder<ConfiguredFeature<?, ?>> holder = level.registryAccess().lookupOrThrow(Registries.CONFIGURED_FEATURE).get(megaFeatureKey).orElse(null);
/*  93 */       if (holder != null) {
/*  94 */         for (int dx = 0; dx >= -1; dx--) {
/*  95 */           for (int dz = 0; dz >= -1; dz--) {
/*  96 */             if (isTwoByTwoSapling(state, (BlockGetter)level, pos, dx, dz)) {
/*  97 */               ConfiguredFeature<?, ?> configuredFeature = (ConfiguredFeature<?, ?>)holder.value();
/*     */               
...
/*     */               
/* 105 */               if (configuredFeature.place((WorldGenLevel)level, generator, random, pos.offset(dx, 0, dz))) {
/* 106 */                 return true;
/*     */               }
...
/*     */     } 
/*     */     
/* 119 */     ResourceKey<ConfiguredFeature<?, ?>> featureKey = getConfiguredFeature(random, hasFlowers((LevelAccessor)level, pos));
/* 120 */     if (featureKey == null) {
/* 121 */       return false;
/*     */     }
/*     */     
/* 124 */     Holder<ConfiguredFeature<?, ?>> featureHolder = level.registryAccess().lookupOrThrow(Registries.CONFIGURED_FEATURE).get(featureKey).orElse(null);
...
/*     */     
/* 134 */     if (feature.place((WorldGenLevel)level, generator, random, pos)) {
...
/* 147 */     return (level.getBlockState(pos.offset(ox, 0, oz)).is(block) && 
/* 148 */       level.getBlockState(pos.offset(ox + 1, 0, oz)).is(block) && 
/* 149 */       level.getBlockState(pos.offset(ox, 0, oz + 1)).is(block) && 
/* 150 */       level.getBlockState(pos.offset(ox + 1, 0, oz + 1)).is(block));
/*     */   }
/*     */   
/*     */   private boolean hasFlowers(LevelAccessor level, BlockPos pos) {
/* 154 */     for (BlockPos p : (Iterable<BlockPos>)BlockPos.MutableBlockPos.betweenClosed(pos.below().north(2).west(2), pos.above().south(2).east(2))) {
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/level/block/grower/TreeGrower.class
