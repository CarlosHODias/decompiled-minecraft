...
/*     */   static {
/*  31 */     CODEC = RecordCodecBuilder.create(i -> i.group((App)LootPoolEntries.CODEC.listOf().fieldOf("entries").forGetter(()), (App)LootItemCondition.DIRECT_CODEC.listOf().optionalFieldOf("conditions", List.of()).forGetter(()), (App)LootItemFunctions.ROOT_CODEC.listOf().optionalFieldOf("functions", List.of()).forGetter(()), (App)NumberProviders.CODEC.fieldOf("rolls").forGetter(()), (App)NumberProviders.CODEC.fieldOf("bonus_rolls").orElse(ConstantValue.exactly(0.0F)).forGetter(())).apply((Applicative)i, LootPool::new));
/*     */   }
/*     */ 
/*     */   
...
/*  59 */     List<LootPoolEntry> validEntries = com.google.common.collect.Lists.newArrayList();
...
/* 106 */       ((LootItemCondition)this.conditions.get(i)).validate(output.forChild((ProblemReporter.PathElement)new ProblemReporter.IndexedFieldPathElement("conditions", i)));
/*     */     }
/*     */     
/* 109 */     for (int k = 0; k < this.functions.size(); k++) {
/* 110 */       ((LootItemFunction)this.functions.get(k)).validate(output.forChild((ProblemReporter.PathElement)new ProblemReporter.IndexedFieldPathElement("functions", k)));
/*     */     }
/*     */     
/* 113 */     for (int j = 0; j < this.entries.size(); j++) {
/* 114 */       ((LootPoolEntryContainer)this.entries.get(j)).validate(output.forChild((ProblemReporter.PathElement)new ProblemReporter.IndexedFieldPathElement("entries", j)));
/*     */     }
/*     */     
/* 117 */     this.rolls.validate(output.forChild((ProblemReporter.PathElement)new ProblemReporter.FieldPathElement("rolls")));
/* 118 */     this.bonusRolls.validate(output.forChild((ProblemReporter.PathElement)new ProblemReporter.FieldPathElement("bonus_rolls")));
/*     */   }
/*     */   
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/level/storage/loot/LootPool.class
