...
/*  35 */     return new TargetPoint(quantizeCoord(temperature), quantizeCoord(humidity), quantizeCoord(continentalness), quantizeCoord(erosion), quantizeCoord(depth), quantizeCoord(weirdness));
/*     */   }
/*     */   
...
/*  39 */     return new ParameterPoint(Parameter.point(temperature), Parameter.point(humidity), Parameter.point(continentalness), Parameter.point(erosion), Parameter.point(depth), Parameter.point(weirdness), quantizeCoord(offset));
/*     */   }
/*     */   
...
/*  43 */     return new ParameterPoint(temperature, humidity, continentalness, erosion, depth, weirdness, quantizeCoord(offset));
/*     */   }
/*     */   
/*     */   public static long quantizeCoord(float coord) {
/*  47 */     return (long)(coord * 10000.0F);
/*     */   }
/*     */   
/*     */   public static float unquantizeCoord(long coord) {
/*  51 */     return (float)coord / 10000.0F;
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/*  79 */         this.parameterSpace = parameterSpace.<Climate.Parameter>toArray(new Climate.Parameter[0]);
/*     */       }
/*     */       
...
/*     */ 
/*     */       
/*     */       protected Leaf<T> search(long[] target, Leaf<T> candidate, Climate.DistanceMetric<T> distanceMetric) {
/* 108 */         return this;
/*     */       }
/*     */     }
/*     */     
...
/* 116 */         this(Climate.RTree.buildParameterSpace(children), children);
/*     */       }
/*     */       
...
/* 121 */         this.children = children.<Climate.RTree.Node<T>>toArray((Climate.RTree.Node<T>[])new Climate.RTree.Node[0]);
/*     */       }
/*     */ 
/*     */       
/*     */       protected Climate.RTree.Leaf<T> search(long[] target, Climate.RTree.Leaf<T> candidate, Climate.DistanceMetric<T> distanceMetric) {
/* 126 */         long minDistance = (candidate == null) ? Long.MAX_VALUE : distanceMetric.distance(candidate, target);
...
/*     */       }
/*     */       
/* 154 */       List<Leaf<T>> leaves = (List<Leaf<T>>)values.stream().map(p -> new Leaf((Climate.ParameterPoint)p.getFirst(), p.getSecond())).collect(Collectors.toCollection(java.util.ArrayList::new));
/*     */       
/* 156 */       return new RTree<>(build(dimensions, (List)leaves));
/*     */     }
/*     */     
...
/*     */ 
/*     */       
/* 182 */       long minCost = Long.MAX_VALUE;
...
/*     */       } 
/*     */       
/* 202 */       sort((List)minBuckets, dimensions, minDimension, true);
/*     */       
/* 204 */       return new SubTree<>((List<? extends Node<T>>)minBuckets.stream().map(b -> build(dimensions, Arrays.asList((Node<?>[])b.children))).collect(Collectors.toList()));
/*     */     }
/*     */     
...
/* 210 */         comparator = comparator.thenComparing((Comparator)comparator((dimension + d) % dimensions, absolute));
/*     */       }
...
/* 263 */           bounds.set(i, child.parameterSpace[i].span(bounds.get(i)));
/*     */         }
/*     */       } 
/* 266 */       return bounds;
/*     */     }
...
/* 271 */       Leaf<T> leaf = this.root.search(targetArray, this.lastResult.get(), distanceMetric);
/* 272 */       this.lastResult.set(leaf);
/* 273 */       return leaf.value;
/*     */     } }
/*     */   static abstract class Node<T> {
/*     */     protected final Climate.Parameter[] parameterSpace;
/*     */     protected Node(List<Climate.Parameter> parameterSpace) { this.parameterSpace = parameterSpace.<Climate.Parameter>toArray(new Climate.Parameter[0]); } protected abstract Climate.RTree.Leaf<T> search(long[] param1ArrayOflong, Climate.RTree.Leaf<T> param1Leaf, Climate.DistanceMetric<T> param1DistanceMetric); protected long distance(long[] target) { long distance = 0L; for (int i = 0; i < 7; i++) distance += Mth.square(this.parameterSpace[i].distance(target[i]));  return distance; } public String toString() { return Arrays.toString((Object[])this.parameterSpace); }
...
/* 301 */       return findValueIndex(target);
/*     */     }
/*     */     
/*     */     @VisibleForTesting
...
/*     */ 
/*     */       
/* 309 */       Pair<Climate.ParameterPoint, T> first = iterator.next();
...
/* 314 */         Pair<Climate.ParameterPoint, T> parameter = iterator.next();
...
/* 325 */       return findValueIndex(target, Climate.RTree.Node::distance);
/*     */     }
/*     */     
/*     */     protected T findValueIndex(Climate.TargetPoint target, Climate.DistanceMetric<T> distanceMetric) {
/* 329 */       return this.index.search(target, distanceMetric);
/*     */     } }
/*     */   public static final class TargetPoint extends Record { private final long temperature; private final long humidity;
...
/*     */     
/*     */     static {
/* 353 */       CODEC = RecordCodecBuilder.create(i -> i.group((App)Climate.Parameter.CODEC.fieldOf("temperature").forGetter(()), (App)Climate.Parameter.CODEC.fieldOf("humidity").forGetter(()), (App)Climate.Parameter.CODEC.fieldOf("continentalness").forGetter(()), (App)Climate.Parameter.CODEC.fieldOf("erosion").forGetter(()), (App)Climate.Parameter.CODEC.fieldOf("depth").forGetter(()), (App)Climate.Parameter.CODEC.fieldOf("weirdness").forGetter(()), (App)Codec.floatRange(0.0F, 1.0F).fieldOf("offset").xmap(Climate::quantizeCoord, Climate::unquantizeCoord).forGetter(())).apply((Applicative)i, ParameterPoint::new));
/*     */     }
/*     */ 
/*     */ 
/*     */ 
...
/* 387 */       this.min = min; this.max = max; } public final int hashCode() { // Byte code:
/*     */       //   0: aload_0
...
/* 387 */       //   0	8	1	o	Ljava/lang/Object; } public long min() { return this.min; } public long max() { return this.max; } static {
/* 388 */       CODEC = ExtraCodecs.intervalCodec(Codec.floatRange(-2.0F, 2.0F), "min", "max", (min, max) -> (min.compareTo(max) > 0) ? DataResult.error(()) : DataResult.success(new Parameter(Climate.quantizeCoord(min), Climate.quantizeCoord(max))), p -> Climate.unquantizeCoord(p.min()), p -> Climate.unquantizeCoord(p.max()));
/*     */     }
/*     */ 
/*     */ 
/*     */ 
...
/*     */     
/*     */     public String toString() {
/* 415 */       return (this.min == this.max) ? String.format(Locale.ROOT, "%d", new Object[] { this.min }) : String.format(Locale.ROOT, "[%d-%d]", new Object[] { this.min, this.max });
/*     */     }
/*     */     
...
/* 438 */       return (other == null) ? this : new Parameter(Math.min(this.min, other.min()), Math.max(this.max, other.max()));
/*     */     }
/*     */   }
/*     */   
/*     */   public static Sampler empty() {
/* 443 */     DensityFunction zero = DensityFunctions.zero();
...
/* 464 */           (float)this.temperature.compute((DensityFunction.FunctionContext)context), 
/* 465 */           (float)this.humidity.compute((DensityFunction.FunctionContext)context), 
/* 466 */           (float)this.continentalness.compute((DensityFunction.FunctionContext)context), 
/* 467 */           (float)this.erosion.compute((DensityFunction.FunctionContext)context), 
/* 468 */           (float)this.depth.compute((DensityFunction.FunctionContext)context), 
/* 469 */           (float)this.weirdness.compute((DensityFunction.FunctionContext)context));
/*     */     }
/*     */ 
/*     */     
...
/* 519 */       Climate.TargetPoint zeroDepthTargetPoint = new Climate.TargetPoint(
/* 520 */           targetPoint.temperature(), 
/* 521 */           targetPoint.humidity(), 
/* 522 */           targetPoint.continentalness(), 
/* 523 */           targetPoint.erosion(), 0L, 
/*     */           
/* 525 */           targetPoint.weirdness());
/*     */ 
/*     */       
/* 528 */       long minFitness = Long.MAX_VALUE;
...
/*     */     } } private static final class Result extends Record { private final BlockPos location; private final long fitness; private Result(BlockPos location, long fitness) { this.location = location; this.fitness = fitness; } public final String toString() { // Byte code:
/*     */       //   0: aload_0
/*     */       //   1: <illegal opcode> toString : (Lnet/minecraft/world/level/biome/Climate$SpawnFinder$Result;)Ljava/lang/String;
/*     */       //   6: areturn
/*     */       // Line number table:
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/level/biome/Climate.class
