...
/*  74 */     return findBiomeHorizontal(x, y, z, searchRadius, 1, allowed, random, false, sampler);
/*     */   }
/*     */   
...
/*  78 */     Set<Holder<Biome>> candidateBiomes = (Set<Holder<Biome>>)possibleBiomes().stream()
/*  79 */       .filter(allowed)
...
/*     */     
/*  89 */     for (BlockPos.MutableBlockPos sampleColumn : (Iterable<BlockPos.MutableBlockPos>)BlockPos.spiralAround(BlockPos.ZERO, sampleRadius, Direction.EAST, Direction.SOUTH)) {
...
/* 121 */     int found = 0;
/*     */     
/* 123 */     int startRadius = findClosest ? 0 : noiseRadius;
/* 124 */     for (int currentRadius = startRadius; currentRadius <= noiseRadius; currentRadius += skipSteps) {
/* 125 */       for (int z = (SharedConstants.DEBUG_ONLY_GENERATE_HALF_THE_WORLD || SharedConstants.debugGenerateSquareTerrainWithoutNoise) ? 0 : -currentRadius; z <= currentRadius; z += skipSteps) {
/* 126 */         boolean zEdge = (Math.abs(z) == currentRadius);
/* 127 */         for (int x = -currentRadius; x <= currentRadius; x += skipSteps) {
/* 128 */           if (findClosest) {
/*     */             
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/level/biome/BiomeSource.class
