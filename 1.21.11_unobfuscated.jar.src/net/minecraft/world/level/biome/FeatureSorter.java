...
/*  28 */       //   0	8	1	o	Ljava/lang/Object; } public StepFeatureData(List<PlacedFeature> features, ToIntFunction<PlacedFeature> indexMapping) { this.features = features; this.indexMapping = indexMapping; } public final int hashCode() {
/*     */       // Byte code:
/*     */       //   0: aload_0
...
/*  33 */       this(features, Util.createIndexIdentityLookup(features));
/*     */     }
/*     */   }
/*     */ 
/*     */   
...
/*  43 */     Comparator<FeatureData> featureDataComparator = Comparator.<FeatureData>comparingInt(FeatureData::step).thenComparingInt(FeatureData::featureIndex);
/*     */     
/*  45 */     Map<FeatureData, Set<FeatureData>> edges = new TreeMap<>(featureDataComparator);
/*     */     
/*  47 */     int maxStep = 0;
/*  48 */     for (T featureSource : featureSources) {
/*  49 */       List<FeatureData> featureList = Lists.newArrayList();
/*  50 */       List<HolderSet<PlacedFeature>> featuresForStep = featureGetter.apply(featureSource);
/*  51 */       maxStep = Math.max(maxStep, featuresForStep.size());
/*  52 */       for (int i = 0; i < featuresForStep.size(); i++) {
/*  53 */         for (Holder<PlacedFeature> featureSupplier : featuresForStep.get(i)) {
...
/*     */         } 
/*     */       } 
/*  58 */       for (int j = 0; j < featureList.size(); j++) {
/*  59 */         Set<FeatureData> data = edges.computeIfAbsent(featureList.get(j), k -> new TreeSet(featureDataComparator));
/*  60 */         if (j < featureList.size() - 1) {
/*  61 */           data.add(featureList.get(j + 1));
/*     */         }
/*     */       } 
/*     */     } 
/*     */     
/*  66 */     Set<FeatureData> discovered = new TreeSet<>(featureDataComparator);
/*  67 */     Set<FeatureData> currentlyVisiting = new TreeSet<>(featureDataComparator);
...
/*     */       }
/*     */       
/*  78 */       java.util.Objects.requireNonNull(sortedFeatures); if (Graph.depthFirstSearch(edges, discovered, currentlyVisiting, sortedFeatures::add, feature)) {
/*  79 */         if (tryReducingError) {
/*  80 */           int lastSize; List<T> reducedSources = new ArrayList<>(featureSources);
/*     */ 
/*     */           
/*     */           do {
...
/*     */     } 
/*     */     
/* 106 */     java.util.Collections.reverse(sortedFeatures);
/*     */     
...
/* 111 */       List<PlacedFeature> featuresInStep = (List<PlacedFeature>)sortedFeatures.stream().filter(p -> (p.step() == finalStep)).map(FeatureData::feature).collect(java.util.stream.Collectors.toList());
/* 112 */       features.add(new StepFeatureData(featuresInStep));
/*     */     } 
/* 114 */     return (List<StepFeatureData>)features.build();
/*     */   }
/*     */ }


/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/level/biome/FeatureSorter.class
