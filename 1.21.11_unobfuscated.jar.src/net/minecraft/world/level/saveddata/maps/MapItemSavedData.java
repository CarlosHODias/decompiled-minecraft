...
/*     */   
/*     */   static {
/*  55 */     CODEC = RecordCodecBuilder.create(i -> i.group((App)Level.RESOURCE_KEY_CODEC.fieldOf("dimension").forGetter(()), (App)Codec.INT.fieldOf("xCenter").forGetter(()), (App)Codec.INT.fieldOf("zCenter").forGetter(()), (App)Codec.BYTE.optionalFieldOf("scale", (byte)0).forGetter(()), (App)Codec.BYTE_BUFFER.fieldOf("colors").forGetter(()), (App)Codec.BOOL.optionalFieldOf("trackingPosition", true).forGetter(()), (App)Codec.BOOL.optionalFieldOf("unlimitedTracking", false).forGetter(()), (App)Codec.BOOL.optionalFieldOf("locked", false).forGetter(()), (App)MapBanner.CODEC.listOf().optionalFieldOf("banners", List.of()).forGetter(()), (App)MapFrame.CODEC.listOf().optionalFieldOf("frames", List.of()).forGetter(())).apply((Applicative)i, MapItemSavedData::new));
/*     */   }
/*     */   public static final class MapPatch extends Record { private final int startX; private final int startY; private final int width; private final int height;
...
/*     */     }
/*     */     
/*  68 */     public MapPatch(int startX, int startY, int width, int height, byte[] mapColors) { this.startX = startX; this.startY = startY; this.width = width; this.height = height; this.mapColors = mapColors; } public final boolean equals(Object o) { // Byte code:
/*     */       //   0: aload_0
/*     */       //   1: aload_1
...
/*  73 */         MapPatch patch = optional.get();
...
/* 111 */     private int maxDirtyX = 127;
/* 112 */     private int maxDirtyY = 127;
...
/* 154 */         return (Packet<?>)new ClientboundMapItemDataPacket(id, MapItemSavedData.this.scale, MapItemSavedData.this.locked, decorations, patch);
/*     */       }
/*     */       
/* 157 */       return null;
/*     */     }
...
/* 176 */       this.dirtyDecorations = true;
/*     */     }
/*     */   }
/*     */ 
/*     */ 
...
/*     */ 
/*     */   
/* 186 */   public byte[] colors = new byte[16384];
/*     */   public final boolean locked;
/* 188 */   private final List<HoldingPlayer> carriedBy = Lists.newArrayList();
/* 189 */   private final Map<Player, HoldingPlayer> carriedByPlayers = Maps.newHashMap();
/* 190 */   private final Map<String, MapBanner> bannerMarkers = Maps.newHashMap();
/* 191 */   private final Map<String, MapDecoration> decorations = Maps.newLinkedHashMap();
/* 192 */   private final Map<String, MapFrame> frameMarkers = Maps.newHashMap();
/*     */   private int trackedDecorationCount;
/*     */   
/*     */   public static SavedDataType<MapItemSavedData> type(MapId id) {
/* 196 */     return new SavedDataType(
/* 197 */         id.key(), () -> { throw new IllegalStateException("Should never create an empty map saved data"); }, CODEC, DataFixTypes.SAVED_DATA_MAP_DATA);
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 223 */       addDecoration(banner.getDecoration(), null, banner.getId(), banner.pos().getX(), banner.pos().getZ(), 180.0D, banner.name().orElse(null));
/*     */     } 
/*     */     
...
/* 255 */     return new MapItemSavedData(0, 0, scale, false, false, isLocked, dimension);
/*     */   }
/*     */   
...
/* 278 */     return createFresh(this.centerX, this.centerZ, (byte)Mth.clamp(this.scale + 1, 0, 4), this.trackingPosition, this.unlimitedTracking, this.dimension);
/*     */   }
/*     */   
/*     */   private static Predicate<ItemStack> mapMatcher(ItemStack mapStack) {
/* 282 */     MapId mapId = (MapId)mapStack.get(DataComponents.MAP_ID);
/* 283 */     return stack -> (stack == mapStack) ? true : (
/*     */ 
/*     */ 
/*     */       
/* 287 */       (stack.is(mapStack.getItem()) && Objects.equals(mapId, stack.get(DataComponents.MAP_ID))));
/*     */   }
/*     */ 
/*     */   
...
/* 304 */       HoldingPlayer otherHoldingPlayer = this.carriedBy.get(i);
...
/* 313 */         addDecoration(MapDecorationTypes.PLAYER, (LevelAccessor)otherPlayer.level(), otherPlayerName, otherPlayer.getX(), otherPlayer.getZ(), otherPlayer.getYRot(), null);
/*     */       } 
/*     */       
...
/* 324 */       MapFrame existingFrame = this.frameMarkers.get(MapFrame.frameId(pos));
/*     */ 
/*     */       
...
/* 331 */       addDecoration(MapDecorationTypes.FRAME, (LevelAccessor)tickingPlayer.level(), getFrameKey(frame.getId()), pos.getX(), pos.getZ(), (frame.getDirection().get2DDataValue() * 90), null);
/* 332 */       MapFrame oldFrame = this.frameMarkers.put(mapFrame.getId(), mapFrame);
...
/*     */             if (!this.decorations.containsKey(tickingPlayer)) {
/*     */               addDecoration(entry.type(), (LevelAccessor)tickingPlayer.level(), tickingPlayer, entry.x(), entry.z(), entry.rotation(), null);
/*     */             }
/*     */           });
/*     */     }
/*     */   }
...
/* 363 */     MapDecoration decoration = this.decorations.remove(string);
...
/* 396 */     MapDecoration previousDecoration = this.decorations.put(key, newDecoration);
...
/* 414 */       return (typeAndRotation == null) ? null : new MapDecorationLocation((Holder<MapDecorationType>)typeAndRotation.getFirst(), clampedXDeltaFromCenter, clampedYDeltaFromCenter, (Byte)typeAndRotation.getSecond());
/*     */     } 
/*     */     
...
/* 426 */       return Pair.of(type, calculateRotation(level, yRot));
/*     */     }
...
/* 430 */       return null;
/*     */     }
/* 432 */     return Pair.of(outsideMapDecorationType, (byte)0);
/*     */   }
/*     */   
...
/*     */       // Local variable table:
/*     */       //   start	length	slot	name	descriptor
/* 472 */       //   0	7	0	this	Lnet/minecraft/world/level/saveddata/maps/MapItemSavedData$MapDecorationLocation; } public Holder<MapDecorationType> type() { return this.type; } public final int hashCode() { // Byte code:
/*     */       //   0: aload_0
...
/* 472 */       //   0	8	1	o	Ljava/lang/Object; } public byte x() { return this.x; } public byte y() { return this.y; } public byte rot() { return this.rot; }
/*     */      }
/*     */   public Packet<?> getUpdatePacket(MapId id, Player player) {
/* 475 */     HoldingPlayer holdingPlayer = this.carriedByPlayers.get(player);
/*     */     
...
/* 493 */     this.carriedBy.forEach(HoldingPlayer::markDecorationsDirty);
/*     */   }
/*     */   
/*     */   public HoldingPlayer getHoldingPlayer(Player player) {
/* 497 */     HoldingPlayer holdingPlayer = this.carriedByPlayers.get(player);
/*     */     
...
/* 516 */       MapBanner banner = MapBanner.fromWorld((BlockGetter)level, pos);
/* 517 */       if (banner == null) {
/* 518 */         return false;
/*     */       }
/*     */       
...
/* 527 */         addDecoration(banner.getDecoration(), level, banner.getId(), xPos, zPos, 180.0D, banner.name().orElse(null));
/* 528 */         setDirty();
/* 529 */         return true;
/*     */       } 
/*     */     } 
...
/* 537 */       MapBanner expected = iterator.next();
...
/* 550 */     return this.bannerMarkers.values();
/*     */   }
/*     */   
...
/* 586 */       MapDecoration decoration = decorations.get(i);
...
/* 595 */     return this.decorations.values();
/*     */   }
/*     */   
/*     */   public boolean isTrackedCountOverLimit(int limit) {
/* 599 */     return (this.trackedDecorationCount >= limit);
/*     */   }
/*     */   
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/level/saveddata/maps/MapItemSavedData.class
