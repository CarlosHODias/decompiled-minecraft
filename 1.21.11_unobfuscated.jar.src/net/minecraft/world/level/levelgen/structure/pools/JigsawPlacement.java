...
/*  52 */       //   0	8	1	o	Ljava/lang/Object; } public MutableObject<VoxelShape> free() { return this.free; } public int depth() { return this.depth; }
/*     */      }
/*     */   
...
/*  62 */     private final SequencedPriorityIterator<JigsawPlacement.PieceState> placing = new SequencedPriorityIterator();
/*     */     
...
/*     */       
/*  81 */       MutableObject<VoxelShape> sourceFree = new MutableObject();
/*     */       
...
/*     */           continue;
/*     */         } 
/* 104 */         Holder<StructureTemplatePool> targetPool = maybeTargetPool.get();
...
/*     */         } 
/*     */         
/* 119 */         boolean attachInsideSource = sourceBB.isInside((Vec3i)targetJigsawPos);
...
/*     */           }
/*     */           
/* 143 */           for (Rotation targetRotation : (Iterable<Rotation>)Rotation.getShuffled(this.random)) {
...
/*     */ 
/*     */               
/* 162 */               expandTo = targetJigsaws.stream().mapToInt(targetJigsaw -> { StructureTemplate.StructureBlockInfo targetJigsawInfo = hackBox.info(); if (!hackBox.isInside((Vec3i)targetJigsawInfo.pos().relative(JigsawBlock.getFrontFacing(targetJigsawInfo.state())))) return 0;  ResourceKey<StructureTemplatePool> childPoolName = hackBox.lookup(hackBox.pool()); Optional<? extends Holder<StructureTemplatePool>> childPool = this.pools.get(childPoolName); Optional<Holder<StructureTemplatePool>> childFallbackPool = childPool.map(()); int childPoolSize = (Integer)childPool.<Integer>map(()).orElse(0), childFallbackSize = (Integer)childFallbackPool.<Integer>map(()).orElse(0); return Math.max(childPoolSize, childFallbackSize); }).max().orElse(0);
/*     */             } 
/*     */             
/* 165 */             for (StructureTemplate.JigsawBlockInfo targetJigsaw : targetJigsaws) {
/* 166 */               int targetBoxY, targetGroundLevelDelta, junctionY; if (!JigsawBlock.canAttach(sourceJigsaw, targetJigsaw)) {
/*     */                 continue;
/*     */               }
/*     */               
/* 170 */               BlockPos targetJigsawLocalPos = targetJigsaw.info().pos();
/*     */               
/* 172 */               BlockPos rawTargetBoxPos = targetJigsawPos.subtract((Vec3i)targetJigsawLocalPos);
...
/*     */               } 
/*     */               
/* 235 */               sourcePiece.addJunction(new JigsawJunction(
/* 236 */                     targetJigsawPos.getX(), junctionY - sourceJigsawLocalY + sourceGroundLevelDelta, 
/*     */                     
/* 238 */                     targetJigsawPos.getZ(), deltaY, targetProjection));
/*     */ 
/*     */ 
/*     */ 
/*     */               
/* 243 */               targetPiece.addJunction(new JigsawJunction(
/* 244 */                     sourceJigsawPos.getX(), junctionY - targetJigsawLocalY + targetGroundLevelDelta, 
/*     */                     
/* 246 */                     sourceJigsawPos.getZ(), -deltaY, sourceProjection));
/*     */ 
/*     */ 
/*     */ 
/*     */               
...
/*     */     
/* 272 */     Rotation centerRotation = Rotation.getRandom((RandomSource)random);
/* 273 */     StructureTemplatePool centerPool = startPool.unwrapKey()
/* 274 */       .flatMap(key -> pools.getOptional(poolAliasLookup.lookup(key)))
/* 275 */       .orElse((StructureTemplatePool)startPool.value());
/*     */     
/* 277 */     StructurePoolElement centerElement = centerPool.getRandomTemplate((RandomSource)random);
/*     */     
...
/* 285 */       Identifier targetJigsawId = startJigsaw.get();
...
/*     */       } 
/* 291 */       anchoredPosition = anchor.get();
/*     */     } else {
/* 293 */       anchoredPosition = position;
/*     */     } 
/*     */     
/* 296 */     BlockPos blockPos1 = anchoredPosition.subtract((Vec3i)position);
/*     */ 
/*     */     
/* 299 */     BlockPos adjustedPosition = position.subtract((Vec3i)blockPos1);
/*     */     
...
/*     */ 
/*     */     
/* 307 */     int bottomY = projectStartToHeightmap.isEmpty() ? 
/* 308 */       adjustedPosition.getY() : (
/* 309 */       position.getY() + chunkGenerator.getFirstFreeHeight(centerX, centerZ, projectStartToHeightmap.get(), heightAccessor, context.randomState()));
/*     */     
...
/*     */     } 
/*     */     
/* 319 */     int centerY = bottomY + blockPos1.getY();
/*     */     
...
/*     */             addPieces(context.randomState(), maxDepth, doExpansionHack, chunkGenerator, structureTemplateManager, heightAccessor, (RandomSource)random, pools, centerPiece, pieces, shape, poolAliasLookup, liquidSettings);
...
/* 356 */     List<StructureTemplate.JigsawBlockInfo> jigsaws = element.getShuffledJigsawBlocks(structureTemplateManager, position, rotation, (RandomSource)random);
...
/*     */     
/* 384 */     Structure.GenerationContext generationContext = new Structure.GenerationContext(level.registryAccess(), generator, generator.getBiomeSource(), level.getChunkSource().randomState(), structureTemplateManager, level.getSeed(), new ChunkPos(position), (LevelHeightAccessor)level, b -> true);
...
/*     */       
/* 390 */       for (StructurePiece piece : (Iterable<StructurePiece>)builder.build().pieces()) {
/* 391 */         if (piece instanceof PoolElementStructurePiece) { PoolElementStructurePiece poolPiece = (PoolElementStructurePiece)piece;
/* 392 */           poolPiece.place((WorldGenLevel)level, structureManager, generator, random, BoundingBox.infinite(), position, keepJigsaws); }
/*     */       
/*     */       } 
/* 395 */       return true;
/*     */     } 
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/level/levelgen/structure/pools/JigsawPlacement.class
