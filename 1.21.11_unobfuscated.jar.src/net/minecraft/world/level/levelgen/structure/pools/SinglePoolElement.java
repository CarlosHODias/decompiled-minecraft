...
/*     */     }
/*  48 */     return Identifier.CODEC.encode(location.get(), ops, prefix);
/*     */   }
/*     */   
/*  51 */   private static final Codec<Either<Identifier, StructureTemplate>> TEMPLATE_CODEC = Codec.of(SinglePoolElement::encodeTemplate, 
/*     */       
/*  53 */       Identifier.CODEC.map(Either::left)); public static final com.mojang.serialization.MapCodec<SinglePoolElement> CODEC; protected final Either<Identifier, StructureTemplate> template; protected final Holder<StructureProcessorList> processors; protected final Optional<LiquidSettings> overrideLiquidSettings;
/*     */   
/*     */   static {
/*  56 */     CODEC = RecordCodecBuilder.mapCodec(i -> i.group((App)templateCodec(), (App)processorsCodec(), (App)projectionCodec(), (App)overrideLiquidSettingsCodec()).apply((Applicative)i, SinglePoolElement::new));
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/*  64 */     return net.minecraft.world.level.levelgen.structure.templatesystem.StructureProcessorType.LIST_CODEC.fieldOf("processors").forGetter(t -> t.processors);
/*     */   }
/*     */   
/*     */   protected static <E extends SinglePoolElement> RecordCodecBuilder<E, Optional<LiquidSettings>> overrideLiquidSettingsCodec() {
/*  68 */     return LiquidSettings.CODEC.optionalFieldOf("override_liquid_settings").forGetter(t -> t.overrideLiquidSettings);
/*     */   }
/*     */   
/*     */   protected static <E extends SinglePoolElement> RecordCodecBuilder<E, Either<Identifier, StructureTemplate>> templateCodec() {
/*  72 */     return TEMPLATE_CODEC.fieldOf("location").forGetter(t -> t.template);
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/*  93 */     Objects.requireNonNull(structureTemplateManager); return (StructureTemplate)this.template.map(structureTemplateManager::getOrCreate, Function.identity());
/*     */   }
/*     */   
...
/*  99 */     List<StructureTemplate.StructureBlockInfo> dataMarkers = com.google.common.collect.Lists.newArrayList();
/* 100 */     for (StructureTemplate.StructureBlockInfo info : (Iterable<StructureTemplate.StructureBlockInfo>)objectArrayList) {
/* 101 */       CompoundTag nbt = info.nbt();
/* 102 */       if (nbt == null) {
/*     */         continue;
/*     */       }
/*     */       
/* 106 */       StructureMode mode = nbt.read("mode", StructureMode.LEGACY_CODEC).orElseThrow();
...
/* 127 */     blocks.sort(HIGHEST_SELECTION_PRIORITY_FIRST);
/*     */   }
/*     */ 
/*     */   
...
/*     */     
/* 141 */     if (template.placeInWorld((ServerLevelAccessor)level, position, referencePos, settings, random, 18)) {
/* 142 */       List<StructureTemplate.StructureBlockInfo> dataMarkers = StructureTemplate.processBlockInfos((ServerLevelAccessor)level, position, referencePos, settings, getDataMarkers(structureTemplateManager, position, rotation, false));
/* 143 */       for (StructureTemplate.StructureBlockInfo dataMarker : dataMarkers) {
/* 144 */         handleDataMarker((LevelAccessor)level, dataMarker, position, rotation, random, chunkBB);
/*     */       }
/*     */       
/* 147 */       return true;
/*     */     } 
...
/* 158 */     settings.addProcessor((StructureProcessor)net.minecraft.world.level.levelgen.structure.templatesystem.BlockIgnoreProcessor.STRUCTURE_BLOCK);
/* 159 */     settings.setFinalizeEntities(true);
/* 160 */     settings.setLiquidSettings(this.overrideLiquidSettings.orElse(liquidSettings));
/* 161 */     if (!keepJigsaws) {
/* 162 */       settings.addProcessor((StructureProcessor)JigsawReplacementProcessor.INSTANCE);
/*     */     }
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/level/levelgen/structure/pools/SinglePoolElement.class
