...
/*  36 */   public static final LiquidSettings DEFAULT_LIQUID_SETTINGS = LiquidSettings.APPLY_WATERLOGGING; public static final int MAX_TOTAL_STRUCTURE_RANGE = 128;
...
/*     */   
/*     */   static {
/*  52 */     CODEC = RecordCodecBuilder.mapCodec(i -> i.group((App)settingsCodec(i), (App)StructureTemplatePool.CODEC.fieldOf("start_pool").forGetter(()), (App)Identifier.CODEC.optionalFieldOf("start_jigsaw_name").forGetter(()), (App)Codec.intRange(0, 20).fieldOf("size").forGetter(()), (App)HeightProvider.CODEC.fieldOf("start_height").forGetter(()), (App)Codec.BOOL.fieldOf("use_expansion_hack").forGetter(()), (App)Heightmap.Types.CODEC.optionalFieldOf("project_start_to_heightmap").forGetter(()), (App)MaxDistance.CODEC.fieldOf("max_distance_from_center").forGetter(()), (App)Codec.list(PoolAliasBinding.CODEC).optionalFieldOf("pool_aliases", List.of()).forGetter(()), (App)DimensionPadding.CODEC.optionalFieldOf("dimension_padding", DEFAULT_DIMENSION_PADDING).forGetter(()), (App)LiquidSettings.CODEC.optionalFieldOf("liquid_settings", DEFAULT_LIQUID_SETTINGS).forGetter(())).apply((Applicative)i, JigsawStructure::new)).validate(JigsawStructure::verifyRange);
/*     */   }
...
/*  92 */     this(settings, startPool, Optional.empty(), maxDepth, startHeight, useExpansionHack, Optional.of(projectStartToHeightmap), new MaxDistance(80), List.of(), DEFAULT_DIMENSION_PADDING, DEFAULT_LIQUID_SETTINGS);
/*     */   }
/*     */   
...
/*  96 */     this(settings, startPool, Optional.empty(), maxDepth, startHeight, useExpansionHack, Optional.empty(), new MaxDistance(80), List.of(), DEFAULT_DIMENSION_PADDING, DEFAULT_LIQUID_SETTINGS);
/*     */   }
/*     */ 
/*     */   
/*     */   public Optional<Structure.GenerationStub> findGenerationPoint(Structure.GenerationContext context) {
/* 101 */     ChunkPos chunkPos = context.chunkPos();
/* 102 */     int height = this.startHeight.sample((RandomSource)context.random(), new net.minecraft.world.level.levelgen.WorldGenerationContext(context.chunkGenerator(), context.heightAccessor()));
...
/*     */ 
/*     */     
/* 127 */     private static final Codec<Integer> HORIZONTAL_VALUE_CODEC = Codec.intRange(1, 128); private static final Codec<MaxDistance> FULL_CODEC; public static final Codec<MaxDistance> CODEC;
/*     */     static {
/* 129 */       FULL_CODEC = RecordCodecBuilder.create(i -> i.group((App)HORIZONTAL_VALUE_CODEC.fieldOf("horizontal").forGetter(MaxDistance::horizontal), (App)ExtraCodecs.intRange(1, DimensionType.Y_SIZE).optionalFieldOf("vertical", DimensionType.Y_SIZE).forGetter(MaxDistance::vertical)).apply((Applicative)i, MaxDistance::new));
/*     */ 
/*     */ 
/*     */ 
/*     */       
/* 134 */       CODEC = Codec.either(FULL_CODEC, HORIZONTAL_VALUE_CODEC).xmap(either -> (MaxDistance)either.map(Function.identity(), MaxDistance::new), distance -> (distance.horizontal == distance.vertical) ? Either.right(distance.horizontal) : Either.left(distance));
/*     */     }
/*     */ 
/*     */ 
/*     */     
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/level/levelgen/structure/structures/JigsawStructure.class
