...
/*  40 */     CODEC = RecordCodecBuilder.mapCodec(i -> i.group((App)Codec.unboundedMap(ResourceKey.codec(Registries.LEVEL_STEM), LevelStem.CODEC).fieldOf("dimensions").forGetter(WorldDimensions::dimensions)).apply((Applicative)i, i.stable(WorldDimensions::new)));
/*     */   }
/*     */ 
/*     */ 
/*     */ 
/*     */   
/*  46 */   private static final Set<ResourceKey<LevelStem>> BUILTIN_ORDER = (Set<ResourceKey<LevelStem>>)ImmutableSet.of(LevelStem.OVERWORLD, LevelStem.NETHER, LevelStem.END);
/*     */ 
/*     */ 
/*     */ 
/*     */ 
...
/*  55 */     LevelStem overworld = dimensions.get(LevelStem.OVERWORLD);
...
/*  62 */     this((Map<ResourceKey<LevelStem>, LevelStem>)
/*  63 */         registry.listElements().collect(java.util.stream.Collectors.toMap(Holder.Reference::key, Holder.Reference::value)));
/*     */   }
/*     */ 
/*     */   
/*     */   public static Stream<ResourceKey<LevelStem>> keysInOrder(Stream<ResourceKey<LevelStem>> knownKeys) {
/*  68 */     return Stream.concat(
/*  69 */         BUILTIN_ORDER.stream(), 
/*  70 */         knownKeys.filter(k -> !BUILTIN_ORDER.contains(k)));
/*     */   }
/*     */ 
/*     */   
...
/*  76 */     Map<ResourceKey<LevelStem>, LevelStem> newDimensions = withOverworld((HolderLookup<DimensionType>)registryLookup, this.dimensions, generator);
...
/*  81 */     LevelStem stem = dimensions.get(LevelStem.OVERWORLD);
/*  82 */     Holder<DimensionType> type = (stem == null) ? (Holder<DimensionType>)dimensionTypes.getOrThrow(BuiltinDimensionTypes.OVERWORLD) : stem.type();
/*     */     
/*  84 */     return withOverworld(dimensions, type, generator);
/*     */   }
/*     */   
...
/*  91 */     return (Map<ResourceKey<LevelStem>, LevelStem>)builder.buildKeepingLast();
/*     */   }
/*     */   
/*     */   public ChunkGenerator overworld() {
/*  95 */     LevelStem stem = this.dimensions.get(LevelStem.OVERWORLD);
/*  96 */     if (stem == null) {
...
/* 103 */     return Optional.ofNullable(this.dimensions.get(key));
/*     */   }
/*     */   
/*     */   public ImmutableSet<ResourceKey<net.minecraft.world.level.Level>> levels() {
/* 107 */     return (ImmutableSet<ResourceKey<net.minecraft.world.level.Level>>)dimensions().keySet().stream().map(Registries::levelStemToLevel).collect(ImmutableSet.toImmutableSet());
/*     */   }
/*     */   
/*     */   public boolean isDebug() {
/* 111 */     return overworld() instanceof DebugLevelSource;
/*     */   }
/*     */   
/*     */   private static PrimaryLevelData.SpecialWorldProperty specialWorldProperty(Registry<LevelStem> registry) {
/* 115 */     return registry.getOptional(LevelStem.OVERWORLD).map(overworld -> {
...
/* 128 */     return isVanillaLike(key, dimension) ? Lifecycle.stable() : Lifecycle.experimental();
/*     */   }
/*     */   
...
/* 147 */       return false;
/*     */     }
/* 149 */     BiomeSource biomeSource = dimension.generator().getBiomeSource(); if (biomeSource instanceof MultiNoiseBiomeSource) { MultiNoiseBiomeSource multiNoiseBiomeSource = (MultiNoiseBiomeSource)biomeSource;
/* 150 */       if (!multiNoiseBiomeSource.stable(net.minecraft.world.level.biome.MultiNoiseBiomeSourceParameterLists.OVERWORLD)) {
/* 151 */         return false;
/*     */       } }
/*     */     
/* 154 */     return true;
...
/* 179 */     Stream<ResourceKey<LevelStem>> knownDimensions = Stream.<ResourceKey<LevelStem>>concat(baseDimensions.registryKeySet().stream(), this.dimensions.keySet().stream()).distinct();
/*     */     
/* 181 */     List<Entry> results = new java.util.ArrayList<>();
/* 182 */     keysInOrder(knownDimensions).forEach(key -> baseDimensions.getOptional(results).or(()).ifPresent(()));
/*     */ 
/*     */ 
/*     */ 
/*     */ 
...
/* 197 */       //   0	8	1	o	Ljava/lang/Object; } public Registry<LevelStem> dimensions() { return this.dimensions; } public PrimaryLevelData.SpecialWorldProperty specialWorldProperty() { return this.specialWorldProperty; }
/*     */ 
/*     */ 
/*     */     
/*     */     public Lifecycle lifecycle() {
/* 202 */       return this.dimensions.registryLifecycle();
/*     */     }
/*     */     
/*     */     public RegistryAccess.Frozen dimensionsRegistryAccess() {
/* 206 */       return new RegistryAccess.ImmutableRegistryAccess(List.of(this.dimensions)).freeze();
/*     */     } }
/*     */ 
/*     */ }


/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/level/levelgen/WorldDimensions.class
