...
/* 31 */         (App)Codec.intRange(0, 32).fieldOf("base_height").forGetter(p -> p.baseHeight), 
/* 32 */         (App)Codec.intRange(0, 24).fieldOf("height_rand_a").forGetter(p -> p.heightRandA), 
/* 33 */         (App)Codec.intRange(0, 24).fieldOf("height_rand_b").forGetter(p -> p.heightRandB));
/*    */   }
/*    */ 
/*    */   
...
/* 52 */     return this.baseHeight + random.nextInt(this.heightRandA + 1) + random.nextInt(this.heightRandB + 1);
/*    */   }
/*    */   
/*    */   private static boolean isDirt(LevelSimulatedReader level, BlockPos pos) {
/* 56 */     return level.isStateAtPosition(pos, state -> 
/* 57 */         (Feature.isDirt(state) && !state.is(Blocks.GRASS_BLOCK) && !state.is(Blocks.MYCELIUM)));
/*    */   }
/*    */ 
/*    */ 
/*    */ 
...
/* 71 */     return placeLog(level, trunkSetter, random, pos, config, Function.identity());
/*    */   }
/*    */   
...
/* 76 */       trunkSetter.accept(pos, stateModifier.apply(config.trunkProvider.getState(random, pos)));
/*    */       
/* 78 */       return true;
/*    */     } 
/* 80 */     return false;
...
/* 84 */     if (isFree(level, (BlockPos)pos)) {
/* 85 */       placeLog(level, trunkSetter, random, (BlockPos)pos, config);
/*    */     }
/*    */   }
/*    */   
/*    */   protected boolean validTreePos(LevelSimulatedReader level, BlockPos pos) {
/* 90 */     return TreeFeature.validTreePos(level, pos);
/*    */   }
/*    */ 
/*    */   
/*    */   public boolean isFree(LevelSimulatedReader level, BlockPos pos) {
/* 95 */     return (validTreePos(level, pos) || level.isStateAtPosition(pos, state -> state.is(BlockTags.LOGS)));
/*    */   }
/*    */   
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/level/levelgen/feature/trunkplacers/TrunkPlacer.class
