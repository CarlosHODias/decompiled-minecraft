...
/* 18 */     BlockColumnConfiguration config = context.config();
...
/*    */     } 
/*    */     
/* 42 */     for (int j = 0; j < layerCount; j++) {
/* 43 */       int count = layerHeights[j];
/* 44 */       if (count != 0) {
/*    */ 
/*    */ 
/*    */         
/* 48 */         BlockColumnConfiguration.Layer layer = config.layers().get(j);
/* 49 */         for (int k = 0; k < count; k++) {
/* 50 */           level.setBlock((BlockPos)placePos, layer.state().getState(random, (BlockPos)placePos), 2);
...
/*    */     
/* 64 */     for (int i = start; i != end && amountToRemove > 0; i += direction) {
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/level/levelgen/feature/BlockColumnFeature.class
