...
/*  30 */     layersByAttribute.forEach((attribute, layers) -> this.attributeSamplers.put(attribute, bakeLayerSampler(attribute, layers)));
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/*  37 */     List<EnvironmentAttributeLayer<Value>> layers = (List)new ArrayList<>(untypedLayers);
/*     */     
/*  39 */     Value constantBaseValue = attribute.defaultValue();
/*  40 */     while (!layers.isEmpty()) { EnvironmentAttributeLayer<Value> environmentAttributeLayer = layers.getFirst(); if (environmentAttributeLayer instanceof EnvironmentAttributeLayer.Constant) { EnvironmentAttributeLayer.Constant<Value> constantLayer = (EnvironmentAttributeLayer.Constant<Value>)environmentAttributeLayer;
/*  41 */         constantBaseValue = constantLayer.applyConstant(constantBaseValue);
...
/*  59 */     addBiomeLayer(builder, (HolderLookup<Biome>)registries.lookupOrThrow(Registries.BIOME), biomeManager);
...
/*  67 */     builder.addConstantLayer(dimensionType.attributes());
/*     */   }
/*     */ 
/*     */ 
/*     */   
/*     */   private static void addBiomeLayer(Builder builder, HolderLookup<Biome> biomes, BiomeManager biomeManager) {
/*  73 */     Stream<EnvironmentAttribute<?>> attributesProvidedByBiomes = biomes.listElements()
/*  74 */       .flatMap(biome -> ((Biome)biome.value()).getAttributes().keySet().stream())
/*  75 */       .distinct();
/*  76 */     attributesProvidedByBiomes.forEach(attribute -> addBiomeLayerForAttribute(builder, attribute, biomeManager));
/*     */   }
/*     */ 
/*     */ 
/*     */   
...
/*     */ 
/*     */   
/*     */   private <Value> ValueSampler<Value> getValueSampler(EnvironmentAttribute<Value> attribute) {
/*  99 */     return (ValueSampler<Value>)this.attributeSamplers.get(attribute);
/*     */   }
/*     */ 
/*     */   
...
/* 108 */     if (sampler == null) {
/* 109 */       return attribute.defaultValue();
/*     */     }
/* 111 */     return sampler.getDimensionValue();
/*     */   }
/*     */ 
/*     */   
...
/* 117 */     if (sampler == null) {
/* 118 */       return attribute.defaultValue();
/*     */     }
/* 120 */     return sampler.getValue(pos, biomeInterpolator);
/*     */   }
/*     */   
/*     */   @VisibleForTesting
...
/* 126 */     return (sampler != null) ? sampler.baseValue : attribute.defaultValue();
/*     */   }
/*     */   
/*     */   @VisibleForTesting
...
/* 162 */       return addLayer(attribute, layer);
/*     */     }
/*     */     
...
/* 166 */       return addLayer(attribute, layer);
/*     */     }
/*     */     
...
/* 170 */       return addLayer(attribute, layer);
/*     */     }
/*     */     
/*     */     private <Value> Builder addLayer(EnvironmentAttribute<Value> attribute, EnvironmentAttributeLayer<Value> layer) {
/* 174 */       ((List<EnvironmentAttributeLayer<Value>>)this.layersByAttribute.computeIfAbsent(attribute, t -> new ArrayList())).add(layer);
/* 175 */       return this;
/*     */     }
/*     */     
/*     */     public Builder addTimelineLayer(Holder<Timeline> timeline, LongSupplier dayTimeGetter) {
/* 179 */       for (EnvironmentAttribute<?> attribute : (Iterable<EnvironmentAttribute<?>>)((Timeline)timeline.value()).attributes()) {
...
/* 186 */       addTimeBasedLayer(attribute, (EnvironmentAttributeLayer.TimeBased<Value>)((Timeline)timeline.value()).createTrackSampler(attribute, dayTimeGetter));
/*     */     }
/*     */     
...
/*     */       }
/* 219 */       Value result = computeValueNotPositional();
...
/*     */       }
/* 229 */       return computeValuePositional(pos, biomeInterpolator);
/*     */     }
/*     */     
/*     */     private Value computeValuePositional(Vec3 pos, SpatialAttributeInterpolator biomeInterpolator) {
/*     */       // Byte code:
/*     */       //   0: aload_0
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/world/attribute/EnvironmentAttributeSystem.class
