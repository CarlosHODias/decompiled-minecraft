...
/*  55 */     Set<String> fullyFilteredEventsSet = new HashSet<>();
...
/*     */ 
/*     */     
/*  82 */     try { InputStream inputStream = Files.newInputStream(Path.of(certificatePath, new String[0]), new java.nio.file.OpenOption[0]); 
/*  83 */       try { certificate = ClientCredentialFactory.createFromCertificate(inputStream, certificatePassword);
/*  84 */         if (inputStream != null) inputStream.close();  } catch (Throwable throwable) { if (inputStream != null) try { inputStream.close(); } catch (Throwable throwable1) { throwable.addSuppressed(throwable1); }   throw throwable; }  } catch (Exception e)
...
/*     */     
/*     */     try {
/*  96 */       client = ((ConfidentialClientApplication.Builder)((ConfidentialClientApplication.Builder)ConfidentialClientApplication.builder(applicationId, (IClientCredential)certificate).sendX5c(true).executorService(workerPool)).authority(String.format(Locale.ROOT, "https://login.microsoftonline.com/%s/", new Object[] { tenantId }))).build();
...
/* 127 */     for (JsonElement element : (Iterable<JsonElement>)events) {
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/server/network/PlayerSafetyServiceTextFilter.class
