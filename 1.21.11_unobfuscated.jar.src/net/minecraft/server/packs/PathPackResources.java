...
/*  79 */     return (IoSupplier<InputStream>)FileUtil.decomposePath(location.getPath()).mapOrElse(decomposedPath -> {
...
/*     */           Path namespaceDir = this.root.resolve(type.getDirectory()).resolve(type);
/*     */           
/*     */           listPath(type, namespaceDir, output, namespace);
/* 105 */         }).ifError(error -> LOGGER.error("Invalid path {}: {}", directory, error.message()));
/*     */   }
/*     */ 
/*     */ 
/*     */   
...
/* 112 */     try { Stream<Path> files = Files.find(targetPath, Integer.MAX_VALUE, PathPackResources::isRegularFile, new java.nio.file.FileVisitOption[0]); 
...
/*     */               if (identifier == null) {
/*     */                 Util.logAndPauseIfInIde(String.format(Locale.ROOT, "Invalid path in pack: %s:%s, ignoring", new Object[] { namespace, resourcePath }));
/*     */               } else {
...
/* 172 */       return new PathPackResources(location, this.content);
/*     */     }
/*     */ 
/*     */     
...
/*     */       }
/*     */       
/* 184 */       List<PackResources> overlayResources = new ArrayList<>(overlays.size());
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/server/packs/PathPackResources.class
