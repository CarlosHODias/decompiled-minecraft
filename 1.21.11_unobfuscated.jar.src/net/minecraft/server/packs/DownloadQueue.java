...
/*     */     
/*     */     static {
/*  42 */       CODEC = RecordCodecBuilder.create(i -> i.group((App)Codec.STRING.fieldOf("name").forGetter(FileInfoEntry::name), (App)Codec.LONG.fieldOf("size").forGetter(FileInfoEntry::size)).apply((Applicative)i, FileInfoEntry::new));
/*     */     } }
...
/*     */     
/*     */     public BatchResult() {
/*  69 */       this(new HashMap<>(), new java.util.HashSet<>());
/*     */     }
/*     */   }
/*     */   
/*     */   public static final class DownloadRequest extends Record { private final URL url;
...
/*     */      }
/*     */   public static final class BatchConfig extends Record { private final HashFunction hashFunction; private final int maxSize; private final Map<String, String> headers; private final Proxy proxy;
...
/*     */ 
/*     */   
/*  91 */   private final ConsecutiveExecutor tasks = new ConsecutiveExecutor((java.util.concurrent.Executor)Util.nonCriticalIoPool(), "download-queue");
/*     */   
...
/*     */           Path targetDir = this.cacheDir.resolve(config.toString()), downloadedFile = null;
/*     */           
/*     */           try {
...
/*     */             config.downloaded.put(config, downloadedFile);
/* 109 */           } catch (Exception e) {
/*     */             LOGGER.error("Failed to download {}", request.url, e);
/*     */ 
/*     */ 
/*     */             
/*     */             config.failed.add(config);
/*     */           } 
/*     */ 
/*     */ 
/*     */           
/*     */           try {
/*     */             this.eventLog.write(new LogEntry(config, request.url.toString(), Instant.now(), Optional.<HashCode>ofNullable(request.hash).map(HashCode::toString), (downloadedFile != null) ? getFileInfo(downloadedFile) : Either.left("download_failed")));
...
/* 140 */     java.util.Objects.requireNonNull(this.tasks); return CompletableFuture.supplyAsync(() -> runDownload(config, requests), this.tasks::schedule);
/*     */   }
/*     */ 
/*     */   
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/server/packs/DownloadQueue.class
