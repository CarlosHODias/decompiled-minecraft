...
/*  53 */     Object2ObjectOpenHashMap<String, LinkFSPath> children = new Object2ObjectOpenHashMap();
/*  54 */     LinkFSPath result = new LinkFSPath(fileSystem, selfName, parent, new PathContents.DirectoryContents((Map<String, LinkFSPath>)children));
/*  55 */     entry.files.forEach((name, linkTarget) -> children.put(name, new LinkFSPath(fileSystem, name, result, new PathContents.FileContents(linkTarget))));
/*     */ 
/*     */     
/*  58 */     entry.children.forEach((name, childEntry) -> children.put(name, buildPath(childEntry, fileSystem, name, result)));
/*     */ 
/*     */     
/*  61 */     children.trim();
/*  62 */     return result;
...
/*     */     }
/* 110 */     String joinedPath = path.collect(Collectors.joining("/"));
/* 111 */     if (joinedPath.equals("/")) {
/* 112 */       return this.root;
/*     */     }
/*     */     
/* 115 */     if (joinedPath.startsWith("/")) {
/* 116 */       LinkFSPath linkFSPath = this.root;
/* 117 */       for (String segment : (Iterable<String>)PATH_SPLITTER.split(joinedPath.substring(1))) {
/* 118 */         if (segment.isEmpty()) {
/* 119 */           throw new IllegalArgumentException("Empty paths not allowed");
/*     */         }
/* 121 */         linkFSPath = linkFSPath.resolveName(segment);
/*     */       } 
/* 123 */       return linkFSPath;
/*     */     } 
/* 125 */     LinkFSPath result = null;
/* 126 */     for (String segment : (Iterable<String>)PATH_SPLITTER.split(joinedPath)) {
...
/* 162 */       //   0	8	1	o	Ljava/lang/Object; } private DirectoryEntry(Map<String, DirectoryEntry> children, Map<String, Path> files) { this.children = children; this.files = files; }
...
/* 174 */         currentEntry = currentEntry.children.computeIfAbsent(segment, n -> new LinkFileSystem.DirectoryEntry());
/*     */       }
...
/* 185 */       return put(path.subList(0, lastIndex), path.get(lastIndex), target);
/*     */     }
/*     */     
/*     */     public FileSystem build(String name) {
/* 189 */       return new LinkFileSystem(name, this.root);
/*     */     } }
/*     */ 
/*     */   
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/server/packs/linkfs/LinkFileSystem.class
