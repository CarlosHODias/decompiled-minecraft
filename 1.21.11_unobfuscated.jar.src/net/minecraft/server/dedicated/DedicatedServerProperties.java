...
/* 126 */     super(settings); this.maxWorldSize = get("max-world-size", v -> Mth.clamp(v, 1, 29999984), 29999984); this.syncChunkWrites = get("sync-chunk-writes", true); this.regionFileComression = get("region-file-compression", "deflate"); this.enableJmxMonitoring = get("enable-jmx-monitoring", false); this.enableStatus = getMutable("enable-status", true); this.hideOnlinePlayers = getMutable("hide-online-players", false); this.entityBroadcastRangePercentage = getMutable("entity-broadcast-range-percentage", v -> Mth.clamp(Integer.parseInt(v), 10, 1000), 100); this.textFilteringConfig = get("text-filtering-config", ""); this.textFilteringVersion = get("text-filtering-version", 0); this.playerIdleTimeout = getMutable("player-idle-timeout", 0); this.statusHeartbeatInterval = getMutable("status-heartbeat-interval", 0); this.whiteList = getMutable("white-list", false); this.enforceSecureProfile = get("enforce-secure-profile", true); this.logIPs = get("log-ips", true); this.pauseWhenEmptySeconds = getMutable("pause-when-empty-seconds", 60);
...
/* 154 */     return new DedicatedServerProperties(loadFromFile(file));
/*     */   }
/*     */ 
/*     */   
/*     */   protected DedicatedServerProperties reload(RegistryAccess registryAccess, Properties properties) {
/* 159 */     return new DedicatedServerProperties(properties);
/*     */   }
/*     */   
...
/* 166 */         return ComponentSerialization.CODEC.parse((DynamicOps)RegistryAccess.EMPTY.createSerializationContext((DynamicOps)JsonOps.INSTANCE), element)
...
/* 233 */     return Integer.toString(permission.level().id());
/*     */   }
/*     */   
/*     */   public WorldDimensions createDimensions(HolderLookup.Provider registries) {
/* 237 */     return this.worldDimensionData.create(registries);
/*     */   }
...
/* 240 */       //   0	7	0	this	Lnet/minecraft/server/dedicated/DedicatedServerProperties$WorldDimensionData; } public JsonObject generatorSettings() { return this.generatorSettings; } public final int hashCode() { // Byte code:
/*     */       //   0: aload_0
...
/* 240 */       //   0	8	1	o	Ljava/lang/Object; } public String levelType() { return this.levelType; }
...
/* 248 */       Holder.Reference<WorldPreset> defaultHolder = (Holder.Reference<WorldPreset>)registryLookup.get(WorldPresets.NORMAL).or(() -> worldPresets.listElements().findAny())
/* 249 */         .orElseThrow(() -> new IllegalStateException("Invalid datapack contents: can't find default preset"));
/*     */ 
/*     */ 
/*     */ 
/*     */       
/* 254 */       Objects.requireNonNull(registryLookup); Holder<WorldPreset> worldPreset = Optional.<Identifier>ofNullable(Identifier.tryParse(this.levelType)).map(id -> ResourceKey.create(Registries.WORLD_PRESET, id)).or(() -> Optional.ofNullable(LEGACY_PRESET_NAMES.get(this.levelType))).flatMap(registryLookup::get)
/* 255 */         .orElseGet(() -> {
...
/* 264 */         RegistryOps<JsonElement> ops = registries.createSerializationContext((DynamicOps)JsonOps.INSTANCE);
/* 265 */         Objects.requireNonNull(DedicatedServerProperties.LOGGER); Optional<FlatLevelGeneratorSettings> parsedSettings = FlatLevelGeneratorSettings.CODEC.parse(new Dynamic((DynamicOps)ops, generatorSettings())).resultOrPartial(DedicatedServerProperties.LOGGER::error);
/* 266 */         if (parsedSettings.isPresent()) {
/* 267 */           return worldDimensions.replaceOverworldGenerator(registries, (ChunkGenerator)new FlatLevelSource(parsedSettings.get()));
/*     */         }
/*     */       } 
/*     */       
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/server/dedicated/DedicatedServerProperties.class
