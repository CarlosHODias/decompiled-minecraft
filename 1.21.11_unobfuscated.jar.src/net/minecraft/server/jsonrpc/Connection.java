...
/*  51 */   private final AtomicInteger transactionId = new AtomicInteger();
/*  52 */   private final Int2ObjectMap<PendingRpcRequest<?>> pendingRequests = Int2ObjectMaps.synchronize((Int2ObjectMap)new Int2ObjectOpenHashMap());
/*     */   
...
/*  55 */     this.clientInfo = ClientInfo.of(CONNECTION_ID_COUNTER.incrementAndGet());
...
/*     */           if (timedOut) {
/*     */             ((PendingRpcRequest)entry.getValue()).resultFuture().completeExceptionally((Throwable)new ReadTimeoutException("RPC method " + String.valueOf(((PendingRpcRequest<Result>)entry.getValue()).method().key().identifier()) + " timed out waiting for response"));
/*     */           }
/*     */           return timedOut;
/*     */         });
/*     */   }
...
/* 134 */     return sendRequest(method, params, true);
/*     */   }
/*     */   
...
/* 139 */     List<JsonElement> jsonParams = (params != null) ? List.<JsonElement>of(Objects.<JsonElement>requireNonNull(((OutgoingRpcMethod)method.value()).encodeParams(params))) : List.<JsonElement>of();
/* 140 */     if (expectReply) {
/* 141 */       CompletableFuture<Result> future = new CompletableFuture<>();
...
/* 145 */       this.channel.writeAndFlush(JsonRPCUtils.createRequest(id, method.key().identifier(), jsonParams));
/* 146 */       return future;
/*     */     } 
...
/* 194 */     return (id.isJsonNull() || GsonHelper.isNumberValue(id) || GsonHelper.isStringValue(id));
/*     */   }
/*     */ 
/*     */   
/*     */   private static boolean isValidResponseId(JsonElement id) {
/* 199 */     return GsonHelper.isNumberValue(id);
/*     */   }
/*     */ 
/*     */ 
/*     */   
...
/*     */       
/*     */       try {
/* 242 */         return this.minecraftApi.submit(() -> ((IncomingRpcMethod)incomingRpcMethod.get()).apply(this.minecraftApi, params, this.clientInfo)).join();
...
/* 256 */     if (request == null) {
/* 257 */       LOGGER.warn("Received unknown response (id: {}): {}", id, result);
/*     */     } else {
...
/* 267 */       if (request != null) {
/* 268 */         request.resultFuture().completeExceptionally((Throwable)new RemoteRpcErrorException(id, error));
/*     */       }
/*     */     } 
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/server/jsonrpc/Connection.class
