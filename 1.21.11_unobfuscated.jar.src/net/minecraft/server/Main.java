...
/*  67 */     net.minecraft.SharedConstants.tryDetectVersion();
/*     */     
...
/*  79 */     ArgumentAcceptingOptionSpec argumentAcceptingOptionSpec1 = parser.accepts("universe").withRequiredArg().defaultsTo(".", (Object[])new String[0]);
/*  80 */     ArgumentAcceptingOptionSpec argumentAcceptingOptionSpec2 = parser.accepts("world").withRequiredArg();
/*  81 */     ArgumentAcceptingOptionSpec argumentAcceptingOptionSpec3 = parser.accepts("port").withRequiredArg().ofType(Integer.class).defaultsTo(-1, (Object[])new Integer[0]);
...
/*  84 */     ArgumentAcceptingOptionSpec argumentAcceptingOptionSpec5 = parser.accepts("pidFile").withRequiredArg().withValuesConvertedBy((joptsimple.ValueConverter)new PathConverter(new joptsimple.util.PathProperties[0]));
/*  85 */     joptsimple.NonOptionArgumentSpec nonOptionArgumentSpec = parser.nonOptions(); try {
/*     */       Dynamic<?> loadedDataTag;
/*     */       WorldStem worldStem;
/*  88 */       OptionSet options = parser.parse(args);
/*     */       
/*  90 */       if (options.has((OptionSpec)abstractOptionSpec)) {
/*  91 */         parser.printHelpOn(System.err);
/*     */         
/*     */         return;
/*     */       } 
/*  95 */       Path pidFilePath = (Path)options.valueOf((OptionSpec)argumentAcceptingOptionSpec5);
...
/*     */ 
/*     */       
/* 103 */       if (options.has((OptionSpec)optionSpecBuilder9)) {
...
/*     */       
/* 121 */       if (options.has((OptionSpec)optionSpecBuilder2)) {
...
/*     */         return;
/*     */       } 
/* 131 */       File universePath = new File((String)options.valueOf((OptionSpec)argumentAcceptingOptionSpec1));
/* 132 */       Services services = Services.create(new YggdrasilAuthenticationService(Proxy.NO_PROXY), universePath);
/*     */       
/* 134 */       String levelName = java.util.Optional.<String>ofNullable((String)options.valueOf((OptionSpec)argumentAcceptingOptionSpec2)).orElse((settings.getProperties()).levelName);
...
/* 152 */             LOGGER.error("Failed to load world data from {}", levelDirectory.oldDataFile(), ex);
...
/*     */       
/* 171 */       boolean safeModeEnabled = options.has((OptionSpec)optionSpecBuilder8);
...
/*     */ 
/*     */         
/* 202 */         worldStem = Util.blockUntilDone(executor -> WorldLoader.load(worldLoadConfig, (), WorldStem::new, (Executor)Util.backgroundExecutor(), executor)).get();
...
/*     */       
/* 211 */       boolean recreateRegionFilesValue = options.has((OptionSpec)optionSpecBuilder7);
/* 212 */       if (options.has((OptionSpec)optionSpecBuilder5) || recreateRegionFilesValue) {
/* 213 */         forceUpgrade(access, data, DataFixers.getDataFixer(), options.has((OptionSpec)optionSpecBuilder6), () -> true, (RegistryAccess)registryHolder, recreateRegionFilesValue);
/*     */       }
/*     */       
/* 216 */       access.saveDataTag((RegistryAccess)registryHolder, data);
/*     */       
/* 218 */       final DedicatedServer dedicatedServer = MinecraftServer.<DedicatedServer>spin(thread -> {
...
/*     */           }
/*     */         };
/* 238 */       shutdownThread.setUncaughtExceptionHandler((Thread.UncaughtExceptionHandler)new DefaultUncaughtExceptionHandler(LOGGER));
...
/* 269 */     return new WorldLoader.DataLoadOutput(new PrimaryLevelData(createLevelSettings, worldOptions, 
/* 270 */           finalDimensions.specialWorldProperty(), lifecycle), 
/* 271 */         finalDimensions.dimensionsRegistryAccess());
/*     */   }
/*     */ 
/*     */   
...
/* 296 */     return new WorldLoader.InitConfig(packConfig, net.minecraft.commands.Commands.CommandSelection.DEDICATED, (PermissionSet)properties.functionPermissions);
/*     */   }
/*     */   
...
/* 313 */           LOGGER.info("{}% completed ({} / {} chunks)...", new Object[] { Mth.floor(done / totalChunks * 100.0F), done, totalChunks });
/*     */         } 
/*     */         
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/server/Main.class
