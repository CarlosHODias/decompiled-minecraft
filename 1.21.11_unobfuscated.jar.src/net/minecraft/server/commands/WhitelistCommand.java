...
/*  32 */         .requires((java.util.function.Predicate)Commands.hasPermission(Commands.LEVEL_ADMINS)))
/*  33 */         .then(
...
/*     */                 
/*     */                 return SharedSuggestionProvider.suggest(list.getPlayers().stream().map(Player::nameAndId).filter(()).map(NameAndId::name), p);
...
/* 105 */         list.remove((net.minecraft.server.players.StoredUserEntry)entry);
...
/*     */     } else {
/* 143 */       source.sendSuccess(() -> Component.translatable("commands.whitelist.list", new Object[] { list.length, String.join(", ", (CharSequence[])list) }), false);
/*     */     } 
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/server/commands/WhitelistCommand.class
