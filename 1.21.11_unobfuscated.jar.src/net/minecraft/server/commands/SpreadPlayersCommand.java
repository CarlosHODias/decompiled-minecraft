...
/*     */   
/*     */   static {
/*  45 */     ERROR_FAILED_TO_SPREAD_TEAMS = new Dynamic4CommandExceptionType((count, x, z, recommended) -> Component.translatableEscape("commands.spreadplayers.failed.teams", new Object[] { count, x, z, recommended }));
/*  46 */     ERROR_FAILED_TO_SPREAD_ENTITIES = new Dynamic4CommandExceptionType((count, x, z, recommended) -> Component.translatableEscape("commands.spreadplayers.failed.entities", new Object[] { count, x, z, recommended }));
/*  47 */     ERROR_INVALID_MAX_HEIGHT = new Dynamic2CommandExceptionType((suppliedMaxHeight, worldMinHeight) -> Component.translatableEscape("commands.spreadplayers.failed.invalid.height", new Object[] { suppliedMaxHeight, worldMinHeight }));
...
/*  89 */       throw ERROR_INVALID_MAX_HEIGHT.create(maxHeight, minY);
/*     */     }
/*     */     
...
/*     */     
/*  98 */     Position[] positions = createInitialPositions(random, respectTeams ? getNumberOfTeams(entities) : entities.size(), minX, minZ, maxX, maxZ);
/*  99 */     spreadPositions(center, spreadDistance, level, random, minX, minZ, maxX, maxZ, maxHeight, positions, respectTeams);
/* 100 */     double distance = setPlayerPositions(entities, level, positions, maxHeight, respectTeams);
/*     */     
/* 102 */     source.sendSuccess(() -> Component.translatable("commands.spreadplayers.success." + (respectTeams ? "teams" : "entities"), new Object[] { positions.length, center.x, center.y, String.format(Locale.ROOT, "%.2f", new Object[] { distance }) }), true);
/* 103 */     return positions.length;
/*     */   }
/*     */   
...
/* 172 */           if (!position.isSafe((BlockGetter)level, maxHeight)) {
...
/* 186 */       if (respectTeams) {
/* 187 */         throw ERROR_FAILED_TO_SPREAD_TEAMS.create(positions.length, center.x, center.y, String.format(Locale.ROOT, "%.2f", new Object[] { minDistance }));
/*     */       }
/* 189 */       throw ERROR_FAILED_TO_SPREAD_ENTITIES.create(positions.length, center.x, center.y, String.format(Locale.ROOT, "%.2f", new Object[] { minDistance }));
/*     */     } 
/*     */   }
/*     */ 
/*     */   
...
/*     */         }
/*     */         
/* 209 */         position = teamPositions.get(playerTeam);
/*     */       } else {
/* 211 */         position = positions[positionIndex++];
/*     */       } 
/*     */       
/* 214 */       entity.teleportTo(level, Mth.floor(position.x) + 0.5D, position.getSpawnY((BlockGetter)level, maxHeight), Mth.floor(position.z) + 0.5D, Set.of(), entity.getYRot(), entity.getXRot(), true);
/*     */       
...
/*     */     }
/*     */     
/* 231 */     avgDistance /= entities.size();
/* 232 */     return avgDistance;
/*     */   }
/*     */   
...
/* 297 */       boolean air2Above = level.getBlockState((BlockPos)pos).isAir();
/* 298 */       pos.move(Direction.DOWN);
/* 299 */       boolean air1Above = level.getBlockState((BlockPos)pos).isAir();
/* 300 */       while (pos.getY() > level.getMinY()) {
/* 301 */         pos.move(Direction.DOWN);
/* 302 */         boolean currentIsAir = level.getBlockState((BlockPos)pos).isAir();
/*     */         
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/server/commands/SpreadPlayersCommand.class
