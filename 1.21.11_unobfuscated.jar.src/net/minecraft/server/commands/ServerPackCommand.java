...
/* 54 */     source.getServer().getConnection().getConnections().forEach(connection -> connection.send(packet));
/*    */   }
/*    */   
/*    */   private static int pushPack(CommandSourceStack source, String url, Optional<UUID> maybeId, Optional<String> maybeHash) {
/* 58 */     UUID id = maybeId.orElseGet(() -> UUID.nameUUIDFromBytes(url.getBytes(StandardCharsets.UTF_8)));
/* 59 */     String hash = maybeHash.orElse("");
/*    */     
/* 61 */     ClientboundResourcePackPushPacket packet = new ClientboundResourcePackPushPacket(id, url, hash, false, null);
/* 62 */     sendToAllConnections(source, (Packet<?>)packet);
/* 63 */     return 0;
/*    */   }
/*    */   
...
/* 68 */     sendToAllConnections(source, (Packet<?>)packet);
/* 69 */     return 0;
/*    */   }
/*    */ }


/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/server/commands/ServerPackCommand.class
