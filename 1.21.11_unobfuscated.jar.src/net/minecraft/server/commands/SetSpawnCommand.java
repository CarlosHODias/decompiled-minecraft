...
/* 34 */         .executes(c -> setSpawn((CommandSourceStack)c.getSource(), Collections.singleton(((CommandSourceStack)c.getSource()).getPlayerOrException()), BlockPos.containing((Position)((CommandSourceStack)c.getSource()).getPosition()), (Coordinates)WorldCoordinates.ZERO_ROTATION)))
/* 35 */         .then((
/* 36 */           (RequiredArgumentBuilder)Commands.argument("targets", (ArgumentType)EntityArgument.players())
/* 37 */           .executes(c -> setSpawn((CommandSourceStack)c.getSource(), EntityArgument.getPlayers(c, "targets"), BlockPos.containing((Position)((CommandSourceStack)c.getSource()).getPosition()), (Coordinates)WorldCoordinates.ZERO_ROTATION)))
/* 38 */           .then((
/* 39 */             (RequiredArgumentBuilder)Commands.argument("pos", (ArgumentType)BlockPosArgument.blockPos())
/* 40 */             .executes(c -> setSpawn((CommandSourceStack)c.getSource(), EntityArgument.getPlayers(c, "targets"), BlockPosArgument.getSpawnablePos(c, "pos"), (Coordinates)WorldCoordinates.ZERO_ROTATION)))
/* 41 */             .then(
...
/* 61 */       source.sendSuccess(() -> Component.translatable("commands.spawnpoint.success.single", new Object[] { pos.getX(), pos.getY(), pos.getZ(), yaw, pitch, dimensionName, ((ServerPlayer)targets.iterator().next()).getDisplayName() }), true);
/*    */     } else {
/* 63 */       source.sendSuccess(() -> Component.translatable("commands.spawnpoint.success.multiple", new Object[] { pos.getX(), pos.getY(), pos.getZ(), yaw, pitch, dimensionName, targets.size() }), true);
/*    */     } 
/*    */     
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/server/commands/SetSpawnCommand.class
