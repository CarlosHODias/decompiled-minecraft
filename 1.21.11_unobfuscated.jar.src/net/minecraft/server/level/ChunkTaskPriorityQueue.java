...
/*    */ {
/* 15 */   public static final int PRIORITY_LEVEL_COUNT = ChunkLevel.MAX_LEVEL + 2;
/* 16 */   private final List<Long2ObjectLinkedOpenHashMap<List<Runnable>>> queuesPerPriority = IntStream.range(0, PRIORITY_LEVEL_COUNT).<Long2ObjectLinkedOpenHashMap<List<Runnable>>>mapToObj(priority -> new Long2ObjectLinkedOpenHashMap()).toList();
/* 17 */   private volatile int topPriorityQueueIndex = PRIORITY_LEVEL_COUNT;
/*    */   
...
/*    */       return;
/*    */     }
/* 29 */     Long2ObjectLinkedOpenHashMap<List<Runnable>> oldQueue = this.queuesPerPriority.get(oldPriority);
/* 30 */     List<Runnable> oldTasks = (List<Runnable>)oldQueue.remove(pos.toLong());
...
/* 37 */       ((List<Runnable>)((Long2ObjectLinkedOpenHashMap)this.queuesPerPriority.get(newPriority)).computeIfAbsent(pos.toLong(), k -> Lists.newArrayList())).addAll(oldTasks);
...
/* 43 */     ((List<Runnable>)((Long2ObjectLinkedOpenHashMap)this.queuesPerPriority.get(level)).computeIfAbsent(chunkPos, p -> Lists.newArrayList())).add(task);
...
/* 49 */       List<Runnable> tasks = (List<Runnable>)queue.get(pos);
/* 50 */       if (tasks == null) {
/*    */         continue;
/*    */       }
/* 53 */       if (unschedule) {
...
/* 70 */     Long2ObjectLinkedOpenHashMap<List<Runnable>> queue = this.queuesPerPriority.get(index);
/* 71 */     long chunkPos = queue.firstLongKey();
/* 72 */     List<Runnable> tasks = (List<Runnable>)queue.removeFirst();
...
/* 80 */     return (this.topPriorityQueueIndex < PRIORITY_LEVEL_COUNT);
/*    */   }
/*    */ 
/*    */   
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/server/level/ChunkTaskPriorityQueue.class
