...
/*  28 */   private final List<GameTestBatchListener> batchListeners = Lists.newArrayList();
/*  29 */   private final List<GameTestInfo> scheduledForRerun = Lists.newArrayList();
/*     */   
...
/*     */     
/*     */     static {
/*  47 */       IN_PLACE = (testInfo -> Optional.<GameTestInfo>ofNullable(testInfo.prepareTestStructure()).map(()));
/*     */     }
/*     */     
...
/*     */   
/*  54 */   public static class Builder { private final GameTestTicker testTicker = GameTestTicker.SINGLETON; private final ServerLevel level;
...
/*  68 */       return new Builder(batches, level);
/*     */     }
/*     */     
/*     */     public static Builder fromInfo(Collection<GameTestInfo> tests, ServerLevel level) {
/*  72 */       return fromBatches(GameTestBatchFactory.fromGameTestInfo().batch(tests), level);
/*     */     }
/*     */     
...
/*     */     
/* 115 */     this.allTestInfos = (List<GameTestInfo>)this.batches.stream().flatMap(batch -> batch.gameTestInfos().stream()).collect(Util.toMutableList());
/* 116 */     testTicker.setRunner(this);
/* 117 */     this.allTestInfos.forEach(info -> info.addListener(new ReportGameListener()));
/*     */   }
/*     */   
/*     */   public List<GameTestInfo> getTestInfos() {
/* 121 */     return this.allTestInfos;
/*     */   }
/*     */   
/*     */   public void start() {
...
/* 138 */     info.getListeners().forEach(listener -> copy.testAddedForRerun(info, info, this));
/*     */     
...
/*     */     
/* 168 */     LOGGER.info("Running test environment '{}' batch {} ({} tests)...", new Object[] { currentBatch.environment().getRegisteredName(), currentBatch.index(), testInfosForThisBatch.size() });
...
/* 239 */     this.batchListeners.add(listener);
/*     */   }
/*     */   
/*     */   private Collection<GameTestInfo> createStructuresForBatch(Collection<GameTestInfo> batch) {
/* 243 */     return batch.stream().map(this::spawn).flatMap(Optional::stream).toList();
/*     */   }
/*     */   
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/gametest/framework/GameTestRunner.class
