...
/*     */ 
/*     */   
/* 172 */   public static final PermissionCheck LEVEL_ALL = (PermissionCheck)PermissionCheck.AlwaysPass.INSTANCE;
/*     */   
/* 174 */   public static final PermissionCheck LEVEL_MODERATORS = (PermissionCheck)new PermissionCheck.Require(Permissions.COMMANDS_MODERATOR);
/*     */   
/* 176 */   public static final PermissionCheck LEVEL_GAMEMASTERS = (PermissionCheck)new PermissionCheck.Require(Permissions.COMMANDS_GAMEMASTER);
/*     */   
/* 178 */   public static final PermissionCheck LEVEL_ADMINS = (PermissionCheck)new PermissionCheck.Require(Permissions.COMMANDS_ADMIN);
/*     */   
/* 180 */   public static final PermissionCheck LEVEL_OWNERS = (PermissionCheck)new PermissionCheck.Require(Permissions.COMMANDS_OWNER);
/*     */   
...
/* 193 */         return (node.getCommand() != null);
/*     */       }
/*     */ 
/*     */       
...
/*     */     };
/*     */   
/* 205 */   private final CommandDispatcher<CommandSourceStack> dispatcher = new CommandDispatcher();
/*     */   
...
/* 342 */     CommandContextBuilder<S> source = context.withSource(sourceOperator.apply((S)context.getSource()));
...
/* 352 */     return command.startsWith("/") ? command.substring(1) : command;
/*     */   }
/*     */   
...
/*     */         }
/*     */       } 
/* 378 */       sender.sendFailure((Component)Component.translatable("command.failed").withStyle(s -> s.withHoverEvent((HoverEvent)new HoverEvent.ShowText((Component)hover))));
/* 379 */       if (SharedConstants.DEBUG_VERBOSE_COMMAND_ERRORS || SharedConstants.IS_RUNNING_IN_IDE) {
/* 380 */         sender.sendFailure((Component)Component.literal(Util.describeError(e)));
...
/*     */       
/* 393 */       return (ContextChain<CommandSourceStack>)ContextChain.tryFlatten(command.getContext().build(commandString))
...
/* 400 */         MutableComponent context = Component.empty().withStyle(ChatFormatting.GRAY).withStyle(s -> s.withClickEvent((ClickEvent)new ClickEvent.SuggestCommand("/" + commandString)));
/*     */ 
/*     */         
/* 403 */         if (cursor > 10) {
...
/* 409 */           context.append((Component)mutableComponent);
/*     */         } 
/* 411 */         context.append((Component)Component.translatable("command.context.here").withStyle(new ChatFormatting[] { ChatFormatting.RED, ChatFormatting.ITALIC }));
/* 412 */         sender.sendFailure((Component)context);
/*     */       } 
/* 414 */       return null;
/*     */     } 
/*     */   }
...
/* 419 */     ExecutionContext<CommandSourceStack> currentContext = CURRENT_EXECUTION_CONTEXT.get();
/* 420 */     boolean isTopContext = (currentContext == null);
/*     */     
/* 422 */     if (isTopContext) {
/*     */       
/* 424 */       GameRules gameRules = context.getLevel().getGameRules();
/* 425 */       int chainLimit = Math.max(1, (Integer)gameRules.get(GameRules.MAX_COMMAND_SEQUENCE_LENGTH));
/* 426 */       int forkLimit = (Integer)gameRules.get(GameRules.MAX_COMMAND_FORKS); 
/* 427 */       try { ExecutionContext<CommandSourceStack> executionContext = new ExecutionContext(chainLimit, forkLimit, Profiler.get()); 
...
/* 440 */     Map<CommandNode<CommandSourceStack>, CommandNode<CommandSourceStack>> playerCommands = new HashMap<>();
/* 441 */     RootCommandNode<CommandSourceStack> root = new RootCommandNode();
/* 442 */     playerCommands.put(this.dispatcher.getRoot(), root);
/* 443 */     fillUsableCommands((CommandNode<CommandSourceStack>)this.dispatcher.getRoot(), (CommandNode<CommandSourceStack>)root, player.createCommandSourceStack(), playerCommands);
/* 444 */     player.connection.send((Packet)new ClientboundCommandsPacket(root, COMMAND_NODE_INSPECTOR));
/*     */   }
/*     */   
...
/* 448 */     for (CommandNode<S> child : (Iterable<CommandNode<S>>)source.getChildren()) {
/* 449 */       if (child.canUse(commandFilter)) {
/* 450 */         ArgumentBuilder<S, ?> builder = child.createBuilder();
/* 451 */         if (builder.getRedirect() != null) {
/* 452 */           builder.redirect(converted.get(builder.getRedirect()));
/*     */         }
...
/* 465 */     return LiteralArgumentBuilder.literal(literal);
/*     */   }
/*     */   
/*     */   public static <T> RequiredArgumentBuilder<CommandSourceStack, T> argument(String name, ArgumentType<T> type) {
/* 469 */     return RequiredArgumentBuilder.argument(name, type);
/*     */   }
/*     */   
...
/* 498 */       return parse.getExceptions().values().iterator().next(); 
...
/* 532 */                 return Optional.of(getOrThrow(id));
/*     */               }
/*     */ 
/*     */               
/*     */               public HolderSet.Named<T> getOrThrow(TagKey<T> id) {
/* 537 */                 Optional<HolderSet.Named<T>> tag = parent().get(id);
/* 538 */                 return tag.orElseGet(() -> HolderSet.emptyNamed((HolderOwner)parent(), id));
/*     */               }
/*     */             };
/*     */         }
/*     */       };
...
/* 546 */     CommandBuildContext context = createValidationContext(VanillaRegistries.createLookup());
...
/*     */     
/* 550 */     dispatcher.findAmbiguities((parent, child, sibling, ambiguities) -> LOGGER.warn("Ambiguity between arguments {} and {} with inputs: {}", new Object[] { dispatcher.getPath(child), dispatcher.getPath(sibling), ambiguities }));
/*     */ 
/*     */ 
/*     */     
/* 554 */     Set<ArgumentType<?>> usedArgumentTypes = ArgumentUtils.findUsedArgumentTypes((CommandNode)root);
/* 555 */     Set<ArgumentType<?>> unregisteredTypes = (Set<ArgumentType<?>>)usedArgumentTypes.stream().filter(arg -> !ArgumentTypeInfos.isClassRecognized(arg.getClass())).collect(Collectors.toSet());
...
/*     */   }
/*     */   
/*     */   public static <T extends net.minecraft.server.permissions.PermissionSetSupplier> PermissionProviderCheck<T> hasPermission(PermissionCheck permission) {
/* 563 */     return new PermissionProviderCheck(permission);
/*     */   }
/*     */ 
/*     */ 
/*     */   
/*     */   public static CommandSourceStack createCompilationContext(PermissionSet compilationPermissions) {
/* 569 */     return new CommandSourceStack(CommandSource.NULL, Vec3.ZERO, Vec2.ZERO, null, compilationPermissions, "", CommonComponents.EMPTY, null, null);
/*     */   }
/*     */   
/*     */   @FunctionalInterface
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/commands/Commands.class
