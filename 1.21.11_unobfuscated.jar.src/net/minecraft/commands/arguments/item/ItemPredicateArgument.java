...
/*  42 */   private static final Collection<String> EXAMPLES = Arrays.asList(new String[] { "stick", "minecraft:stick", "#stick", "#stick{foo:'bar'}" }); private static final DynamicCommandExceptionType ERROR_UNKNOWN_ITEM; private static final DynamicCommandExceptionType ERROR_UNKNOWN_TAG; private static final DynamicCommandExceptionType ERROR_UNKNOWN_COMPONENT; private static final Dynamic2CommandExceptionType ERROR_MALFORMED_COMPONENT; private static final DynamicCommandExceptionType ERROR_UNKNOWN_PREDICATE; private static final Dynamic2CommandExceptionType ERROR_MALFORMED_PREDICATE;
/*     */   static {
/*  44 */     ERROR_UNKNOWN_ITEM = new DynamicCommandExceptionType(id -> Component.translatableEscape("argument.item.id.invalid", new Object[] { id }));
/*  45 */     ERROR_UNKNOWN_TAG = new DynamicCommandExceptionType(tag -> Component.translatableEscape("arguments.item.tag.unknown", new Object[] { tag }));
/*  46 */     ERROR_UNKNOWN_COMPONENT = new DynamicCommandExceptionType(id -> Component.translatableEscape("arguments.item.component.unknown", new Object[] { id }));
/*  47 */     ERROR_MALFORMED_COMPONENT = new Dynamic2CommandExceptionType((type, message) -> Component.translatableEscape("arguments.item.component.malformed", new Object[] { type, message }));
/*  48 */     ERROR_UNKNOWN_PREDICATE = new DynamicCommandExceptionType(id -> Component.translatableEscape("arguments.item.predicate.unknown", new Object[] { id }));
/*  49 */     ERROR_MALFORMED_PREDICATE = new Dynamic2CommandExceptionType((type, message) -> Component.translatableEscape("arguments.item.predicate.malformed", new Object[] { type, message }));
...
/*     */       }
/*     */       
/*  58 */       return new ComponentWrapper(id, itemStack -> itemStack.has(type), 
/*     */ 
/*     */           
/*  61 */           codec.map(expected -> ()));
/*     */     }
/*     */ 
/*     */ 
/*     */ 
...
/*  70 */       return (Predicate<ItemStack>)result.getOrThrow(message -> ItemPredicateArgument.ERROR_MALFORMED_COMPONENT.createWithContext(reader, this.id.toString(), reader));
/*     */     } }
...
/*  76 */       this(
/*  77 */           holder.key().identifier(), ((DataComponentPredicate.Type)
/*  78 */           holder.value()).codec().map(v -> {
/*     */               Objects.requireNonNull(v);
/*     */               return v::matches;
/*     */             }));
/*     */     } public Predicate<ItemStack> decode(ImmutableStringReader reader, Dynamic<?> value) throws CommandSyntaxException {
/*  83 */       DataResult<? extends Predicate<ItemStack>> result = this.type.parse(value);
/*  84 */       return (Predicate<ItemStack>)result.getOrThrow(message -> ItemPredicateArgument.ERROR_MALFORMED_PREDICATE.createWithContext(reader, this.id.toString(), reader));
/*     */     } }
/*     */ 
/*     */   
...
/*     */   }
/*     */   
/*  93 */   private static final Identifier COUNT_ID = Identifier.withDefaultNamespace("count"); private static final Map<Identifier, ComponentWrapper> PSEUDO_COMPONENTS;
/*     */   private static final Map<Identifier, PredicateWrapper> PSEUDO_PREDICATES;
/*     */   
/*     */   static {
/*  97 */     PSEUDO_COMPONENTS = (Map<Identifier, ComponentWrapper>)Stream.<ComponentWrapper>of(new ComponentWrapper(COUNT_ID, itemStack -> true, MinMaxBounds.Ints.CODEC.map(range -> ()))).collect(Collectors.toUnmodifiableMap(ComponentWrapper::id, e -> e));
/*     */ 
/*     */ 
/*     */     
/* 101 */     PSEUDO_PREDICATES = (Map<Identifier, PredicateWrapper>)Stream.<PredicateWrapper>of(new PredicateWrapper(COUNT_ID, MinMaxBounds.Ints.CODEC.map(range -> ()))).collect(Collectors.toUnmodifiableMap(PredicateWrapper::id, e -> e));
/*     */   }
/*     */   public ItemPredicateArgument(CommandBuildContext registries) {
/* 104 */     super(
/* 105 */         ComponentPredicateParser.createGrammar((ComponentPredicateParser.Context<T, ?, ?>)new Context((HolderLookup.Provider)registries))
/* 106 */         .mapResult(predicates -> {
/*     */             Objects.requireNonNull(Util.allOf(predicates));
/*     */             return Util.allOf(predicates)::test;
/*     */           }));
/*     */   } public static ItemPredicateArgument itemPredicate(CommandBuildContext context) {
/* 111 */     return new ItemPredicateArgument(context);
/*     */   }
/*     */   
...
/* 129 */       Holder.Reference<Item> item = (Holder.Reference<Item>)this.items.get(ResourceKey.create(Registries.ITEM, id)).orElseThrow(() -> ItemPredicateArgument.ERROR_UNKNOWN_ITEM.createWithContext(reader, id));
/* 130 */       return itemStack -> itemStack.is((Holder)item);
/*     */     }
/*     */ 
/*     */     
/*     */     public Predicate<ItemStack> forTagType(ImmutableStringReader reader, Identifier id) throws CommandSyntaxException {
/* 135 */       HolderSet<Item> tag = (HolderSet<Item>)this.items.get(TagKey.create(Registries.ITEM, id)).orElseThrow(() -> ItemPredicateArgument.ERROR_UNKNOWN_TAG.createWithContext(reader, id));
...
/* 141 */       ItemPredicateArgument.ComponentWrapper wrapper = ItemPredicateArgument.PSEUDO_COMPONENTS.get(componentId);
/* 142 */       if (wrapper != null) {
/* 143 */         return wrapper;
/*     */       }
/*     */       
/* 146 */       DataComponentType<?> componentType = (DataComponentType)this.components.get(ResourceKey.create(Registries.DATA_COMPONENT_TYPE, componentId))
/* 147 */         .map(Holder::value)
/* 148 */         .orElseThrow(() -> ItemPredicateArgument.ERROR_UNKNOWN_COMPONENT.createWithContext(reader, componentId));
...
/* 154 */       return componentType.decode(reader, RegistryOps.injectRegistryContext(value, this.registries));
/*     */     }
/*     */ 
/*     */     
...
/* 159 */       return componentType.presenceChecker;
/*     */     }
/*     */ 
/*     */     
...
/* 164 */       ItemPredicateArgument.PredicateWrapper wrapper = ItemPredicateArgument.PSEUDO_PREDICATES.get(componentId);
/* 165 */       if (wrapper != null) {
/* 166 */         return wrapper;
/*     */       }
/*     */       
...
/* 171 */         .or(() -> this.components.get(ResourceKey.create(Registries.DATA_COMPONENT_TYPE, componentId)).map(ItemPredicateArgument::createComponentExistencePredicate))
/*     */ 
/*     */ 
/*     */         
...
/* 180 */       return predicateType.decode(reader, RegistryOps.injectRegistryContext(value, this.registries));
/*     */     }
/*     */ 
/*     */     
/*     */     public Stream<Identifier> listElementTypes() {
/* 185 */       return this.items.listElementIds().map(ResourceKey::identifier);
/*     */     }
/*     */ 
/*     */     
/*     */     public Stream<Identifier> listTagTypes() {
/* 190 */       return this.items.listTagIds().map(TagKey::location);
/*     */     }
/*     */ 
/*     */     
/*     */     public Stream<Identifier> listComponentTypes() {
/* 195 */       return Stream.concat(
/* 196 */           ItemPredicateArgument.PSEUDO_COMPONENTS.keySet().stream(), 
/* 197 */           this.components.listElements().filter(e -> !((DataComponentType)e.value()).isTransient()).map(e -> e.key().identifier()));
/*     */     }
/*     */ 
/*     */ 
/*     */     
/*     */     public Stream<Identifier> listPredicateTypes() {
/* 203 */       return Stream.concat(
/* 204 */           ItemPredicateArgument.PSEUDO_PREDICATES.keySet().stream(), 
/* 205 */           this.predicates.listElementIds().map(ResourceKey::identifier));
/*     */     }
/*     */ 
/*     */ 
/*     */     
/*     */     public Predicate<ItemStack> negate(Predicate<ItemStack> value) {
/* 211 */       return value.negate();
/*     */     }
/*     */ 
/*     */     
/*     */     public Predicate<ItemStack> anyOf(List<Predicate<ItemStack>> alternatives) {
/* 216 */       return Util.anyOf(alternatives);
/*     */     }
/*     */   }
/*     */   
/*     */   public static Result getItemPredicate(CommandContext<CommandSourceStack> context, String name) {
/* 221 */     return (Result)context.getArgument(name, Result.class);
/*     */   }
/*     */ 
/*     */   
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/commands/arguments/item/ItemPredicateArgument.class
