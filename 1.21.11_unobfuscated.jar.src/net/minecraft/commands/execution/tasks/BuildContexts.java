...
/*     */   static {
/*  30 */     ERROR_FORK_LIMIT_REACHED = new DynamicCommandExceptionType(limit -> Component.translatableEscape("command.forkLimit", new Object[] { limit }));
/*     */   }
/*     */   @VisibleForTesting
...
/*  48 */     List<T> currentSources = initialSources;
/*     */     
...
/*  62 */             CustomModifierExecutor<T> customModifierExecutor = (CustomModifierExecutor<T>)modifier;
/*  63 */             customModifierExecutor.apply(originalSource, currentSources, currentStage, modifiers, ExecutionControl.create(context, frame));
/*     */             
/*     */             return;
/*     */           } 
/*     */           
...
/*  75 */             for (ExecutionCommandSource executionCommandSource : currentSources) {
/*     */               
/*     */               try {
/*  78 */                 Collection<T> newSources = ContextChain.runModifier(contextToRun, executionCommandSource, (c, s, r) -> {  }, forkedMode);
/*  79 */                 if (objectArrayList1.size() + newSources.size() >= forkLimit) {
/*  80 */                   originalSource.handleError(ERROR_FORK_LIMIT_REACHED.create(forkLimit), forkedMode, context.tracer());
/*     */                   return;
/*     */                 } 
/*  83 */                 objectArrayList1.addAll(newSources);
/*  84 */               } catch (CommandSyntaxException e) {
/*  85 */                 executionCommandSource.handleError(e, forkedMode, context.tracer());
/*  86 */                 if (!forkedMode) {
/*     */                   return;
/*     */                 }
/*     */               } 
...
/* 111 */       CustomCommandExecutor<T> customCommandExecutor = (CustomCommandExecutor<T>)command;
...
/* 114 */         customCommandExecutor.run(executionCommandSource, currentStage, modifiers, executionControl); 
/*     */     } else {
/*     */       List<ExecutionCommandSource> list;
/* 117 */       if (modifiers.isReturn()) {
/*     */         
/* 119 */         ExecutionCommandSource executionCommandSource = (ExecutionCommandSource)objectArrayList.get(0);
/*     */         
/* 121 */         executionCommandSource = executionCommandSource.withCallback(CommandResultCallback.chain(
/* 122 */               executionCommandSource.callback(), 
/* 123 */               frame.returnValueConsumer()));
/*     */         
/* 125 */         list = List.of(executionCommandSource);
/*     */       } 
/*     */ 
/*     */       
/* 129 */       ExecuteCommand<T> action = new ExecuteCommand<>(this.commandInput, modifiers, executeContext);
/* 130 */       ContinuationTask.schedule(context, frame, list, (frame1, entrySource) -> new CommandQueueEntry(frame1, action.bind(entrySource)));
/*     */     } 
/*     */   }
/*     */ 
/*     */ 
...
/* 150 */       super(commandInput, command);
/*     */     }
/*     */ 
/*     */     
...
/* 174 */       execute(this.originalSource, this.sources, context, frame, this.modifiers);
/*     */     }
/*     */   }
/*     */   
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/commands/execution/tasks/BuildContexts.class
