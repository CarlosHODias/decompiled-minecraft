...
/*  53 */       //   0	8	1	o	Ljava/lang/Object; } public byte[] payload() { return this.payload; } public HashCode hash() { return this.hash; }
/*     */      }
/*     */   
...
/*     */     
/*  61 */     for (Iterator<Path> iterator = this.inputFolders.iterator(); iterator.hasNext(); ) { Path input = iterator.next();
/*  62 */       tasks.add(CompletableFuture.supplyAsync(() -> { try { Stream<Path> files = Files.walk(input, new java.nio.file.FileVisitOption[0]); try { CompletableFuture<Void> completableFuture = CompletableFuture.allOf((CompletableFuture<?>[])files.filter(()).map(()).toArray(())); if (files != null)
/*  63 */                     files.close();  return completableFuture; } catch (Throwable throwable) { if (files != null) try { files.close(); } catch (Throwable throwable1)
...
/*  92 */         CompoundTag updated = applyFilters(name, net.minecraft.nbt.NbtUtils.snbtToStructure(input));
...
/*  95 */         NbtIo.writeCompressed(updated, (OutputStream)hos);
...
/* 120 */       super(path.toAbsolutePath().toString(), t);
/*     */     }
/*     */   }
/*     */ }


/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/data/structures/SnbtToNbt.class
