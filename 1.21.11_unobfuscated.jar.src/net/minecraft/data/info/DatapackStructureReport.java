...
/*     */   private static final class Report extends Record { private final Map<ResourceKey<? extends Registry<?>>, DatapackStructureReport.Entry> registries; private final Map<String, DatapackStructureReport.CustomPackEntry> others; public static final Codec<Report> CODEC;
...
/*     */     
/*     */     static {
/*  58 */       CODEC = RecordCodecBuilder.create(i -> i.group((App)Codec.unboundedMap(DatapackStructureReport.REGISTRY_KEY_CODEC, DatapackStructureReport.Entry.CODEC).fieldOf("registries").forGetter(Report::registries), (App)Codec.unboundedMap((Codec)Codec.STRING, DatapackStructureReport.CustomPackEntry.CODEC).fieldOf("others").forGetter(Report::others)).apply((Applicative)i, Report::new));
/*     */     } }
/*     */ 
/*     */ 
/*     */ 
...
/*  71 */         listRegistries(), NON_REGISTRY_ENTRIES);
/*     */ 
/*     */ 
/*     */     
/*  75 */     Path path = this.output.getOutputFolder(PackOutput.Target.REPORTS).resolve("datapack.json");
/*  76 */     return DataProvider.saveStable(cache, (JsonElement)Report.CODEC.encodeStart((DynamicOps)JsonOps.INSTANCE, report).getOrThrow(), path);
/*     */   }
/*     */ 
/*     */   
/*     */   public String getName() {
/*  81 */     return "Datapack Structure";
/*     */   }
/*     */   
/*     */   private void putIfNotPresent(Map<ResourceKey<? extends Registry<?>>, Entry> output, ResourceKey<? extends Registry<?>> key, Entry entry) {
/*  85 */     Entry previous = output.putIfAbsent(key, entry);
/*  86 */     if (previous != null) {
...
/*  92 */     Map<ResourceKey<? extends Registry<?>>, Entry> result = new HashMap<>();
/*     */ 
/*     */     
/*  95 */     BuiltInRegistries.REGISTRY.forEach(entry -> putIfNotPresent(result, result.key(), BUILT_IN_REGISTRY));
/*  96 */     RegistryDataLoader.WORLDGEN_REGISTRIES.forEach(entry -> putIfNotPresent(result, result.key(), UNSTABLE_DYNAMIC_REGISTRY));
/*  97 */     RegistryDataLoader.DIMENSION_REGISTRIES.forEach(entry -> putIfNotPresent(result, result.key(), UNSTABLE_DYNAMIC_REGISTRY));
/*     */ 
/*     */     
/* 100 */     MANUAL_ENTRIES.forEach((key, entry) -> putIfNotPresent(result, result, entry));
/*     */     
/* 102 */     return result;
/*     */   }
/*     */   private static final class Entry extends Record { private final boolean elements; private final boolean tags; private final boolean stable; public static final MapCodec<Entry> MAP_CODEC;
...
/*     */     
/*     */     static {
/* 110 */       MAP_CODEC = RecordCodecBuilder.mapCodec(i -> i.group((App)Codec.BOOL.fieldOf("elements").forGetter(Entry::elements), (App)Codec.BOOL.fieldOf("tags").forGetter(Entry::tags), (App)Codec.BOOL.fieldOf("stable").forGetter(Entry::stable)).apply((Applicative)i, Entry::new));
/*     */     }
/*     */ 
/*     */ 
/*     */ 
...
/*     */ 
/*     */     
/* 124 */     public static final Codec<Format> CODEC = (Codec<Format>)StringRepresentable.fromEnum(Format::values);
/*     */     
...
/*     */     } }
/*     */   private static final class CustomPackEntry extends Record { private final DatapackStructureReport.Format format; private final DatapackStructureReport.Entry entry; public static final Codec<CustomPackEntry> CODEC;
/*     */     
...
/*     */     
/*     */     static {
/* 142 */       CODEC = RecordCodecBuilder.create(i -> i.group((App)DatapackStructureReport.Format.CODEC.fieldOf("format").forGetter(CustomPackEntry::format), (App)DatapackStructureReport.Entry.MAP_CODEC.forGetter(CustomPackEntry::entry)).apply((Applicative)i, CustomPackEntry::new));
/*     */     } }
/*     */ 
/*     */ }


/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/data/info/DatapackStructureReport.class
