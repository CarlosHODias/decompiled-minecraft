...
/*  37 */   private static final Logger LOGGER = LogUtils.getLogger(); private final PackOutput.PathProvider pathProvider; private final Set<ResourceKey<LootTable>> requiredTables; private final List<SubProviderEntry> subProviders; private final CompletableFuture<HolderLookup.Provider> registries;
...
/*  55 */     return this.registries.thenCompose(registries -> run(cache, cache));
/*     */   }
/*     */   
...
/*     */     
/*  63 */     this.subProviders.forEach(subProvider -> ((LootTableSubProvider)subProvider.provider().apply(registries)).generate(()));
/*     */ 
/*     */ 
/*     */ 
/*     */ 
...
/*     */     
/*  81 */     ValidationContext validationContext = new ValidationContext((ProblemReporter)problems, LootContextParamSets.ALL_PARAMS, (HolderGetter.Provider)frozen);
/*     */     
/*  83 */     Sets.SetView setView = Sets.difference(this.requiredTables, mappedRegistry.registryKeySet());
/*     */     
/*  85 */     for (ResourceKey<LootTable> missingTable : (Iterable<ResourceKey<LootTable>>)setView) {
/*  86 */       problems.report(new MissingTableProblem(missingTable));
/*     */     }
/*     */     
/*  89 */     mappedRegistry.listElements().forEach(tableHolder -> ((LootTable)tableHolder.value()).validate(validationContext.setContextKeySet(((LootTable)tableHolder.value()).getParamSet()).enterElement((ProblemReporter.PathElement)new ProblemReporter.RootElementPathElement(tableHolder.key()), tableHolder.key())));
/*     */ 
/*     */ 
/*     */     
...
/*  99 */         .map(entry -> {
/*     */             ResourceKey<LootTable> id = (ResourceKey<LootTable>)cache.getKey();
/*     */             
/*     */             LootTable table = (LootTable)cache.getValue();
/*     */             Path path = this.pathProvider.json(id.identifier());
/*     */             return DataProvider.saveStable(cache, cache, LootTable.DIRECT_CODEC, table, path);
/* 105 */           }).toArray(x$0 -> new CompletableFuture[x$0]));
/*     */   }
/*     */   
/*     */   private static Identifier sequenceIdForLootTable(ResourceKey<LootTable> id) {
/* 109 */     return id.identifier();
/*     */   }
/*     */ 
/*     */   
/*     */   public final String getName() {
/* 114 */     return "Loot Tables";
/*     */   }
...
/* 120 */       return "Missing built-in table: " + String.valueOf(this.id.identifier());
/*     */     } }
/*     */ 
/*     */ }


/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/data/loot/LootTableProvider.class
