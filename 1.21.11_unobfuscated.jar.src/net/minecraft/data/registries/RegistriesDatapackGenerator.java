...
/*    */ 
/*    */   
/*    */   private <T> java.util.Optional<CompletableFuture<?>> dumpRegistryCap(CachedOutput cache, HolderLookup.Provider registries, DynamicOps<JsonElement> writeOps, RegistryDataLoader.RegistryData<T> v) {
...
/*    */           return CompletableFuture.allOf((CompletableFuture<?>[])v.listElements().map(()).toArray(()));
/*    */         });
/*    */   }
/*    */ 
/*    */ 
...
/* 50 */     return (CompletableFuture)codec.encodeStart(ops, value).mapOrElse(result -> DataProvider.saveStable(cache, result, path), error -> CompletableFuture.failedFuture(new IllegalStateException("Couldn't generate file '" + String.valueOf(path) + "': " + error.message())));
/*    */   }
/*    */ 
/*    */ 
/*    */ 
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/data/registries/RegistriesDatapackGenerator.class
