...
/*     */ 
/*     */           
/*     */           long max = runtime.maxMemory(), total = runtime.totalMemory(), free = runtime.freeMemory(), maxMb = max / 1048576L, totalMb = total / 1048576L, freeMb = free / 1048576L;
/*     */ 
/*     */           
/*     */           return "" + free + " bytes (" + free + " MiB) / " + freeMb + " bytes (" + total + " MiB) up to " + totalMb + " bytes (" + max + " MiB)";
/*     */         });
/*     */ 
/*     */     
...
/*  69 */     return String.format(Locale.ROOT, "%d total; %s", new Object[] { selectedArguments.size(), String.join(" ", (Iterable)selectedArguments) });
/*     */   }
/*     */   
/*     */   public void setDetail(String key, String value) {
/*  73 */     this.entries.put(key, value);
/*     */   }
/*     */   
/*     */   public void setDetail(String key, Supplier<String> valueSupplier) {
/*     */     try {
/*  78 */       setDetail(key, valueSupplier.get());
...
/* 102 */     return (float)bytes / 1048576.0F;
/*     */   }
/*     */   
...
/* 108 */       String prefix = String.format(Locale.ROOT, "Memory slot #%d ", new Object[] { memorySlot++ });
/* 109 */       setDetail(prefix + "capacity (MiB)", () -> String.format(Locale.ROOT, "%.2f", new Object[] { sizeInMiB(physicalMemory.getCapacity()) }));
/* 110 */       setDetail(prefix + "clockSpeed (GHz)", () -> String.format(Locale.ROOT, "%.2f", new Object[] { (float)physicalMemory.getClockSpeed() / 1.0E9F }));
...
/* 116 */     setDetail("Virtual memory max (MiB)", () -> String.format(Locale.ROOT, "%.2f", new Object[] { sizeInMiB(virtualMemory.getVirtualMax()) }));
/* 117 */     setDetail("Virtual memory used (MiB)", () -> String.format(Locale.ROOT, "%.2f", new Object[] { sizeInMiB(virtualMemory.getVirtualInUse()) }));
/* 118 */     setDetail("Swap memory total (MiB)", () -> String.format(Locale.ROOT, "%.2f", new Object[] { sizeInMiB(virtualMemory.getSwapTotal()) }));
/* 119 */     setDetail("Swap memory used (MiB)", () -> String.format(Locale.ROOT, "%.2f", new Object[] { sizeInMiB(virtualMemory.getSwapUsed()) }));
/*     */   }
/*     */   
...
/* 130 */       String prefix = String.format(Locale.ROOT, "Graphics card #%d ", new Object[] { gpuIndex++ });
...
/* 133 */       setDetail(prefix + "VRAM (MiB)", () -> String.format(Locale.ROOT, "%.2f", new Object[] { sizeInMiB(graphicsCard.getVRam()) }));
...
/* 146 */     setDetail("Frequency (GHz)", () -> String.format(Locale.ROOT, "%.2f", new Object[] { (float)processorIdentifier.getVendorFreq() / 1.0E9F }));
/*     */     
...
/* 162 */     putSpaceForPath(env, () -> System.getProperty(env));
/*     */   }
/*     */   
...
/*     */     
/*     */     try {
/* 169 */       String path = pathSupplier.get();
/* 170 */       if (path == null) {
...
/* 176 */       setDetail(key, String.format(Locale.ROOT, "available: %.2f, total: %.2f", new Object[] { sizeInMiB(store.getUsableSpace()), sizeInMiB(store.getTotalSpace()) }));
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/SystemReport.class
