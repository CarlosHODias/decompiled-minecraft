...
/*     */   static {
/*  34 */     MAP_CODEC = RecordCodecBuilder.mapCodec(i -> i.group((App)Codec.STRING.fieldOf("nbt").forGetter(NbtContents::getNbtPath), (App)Codec.BOOL.lenientOptionalFieldOf("interpret", false).forGetter(NbtContents::isInterpreting), (App)ComponentSerialization.CODEC.lenientOptionalFieldOf("separator").forGetter(NbtContents::getSeparator), (App)DataSources.CODEC.forGetter(NbtContents::getDataSource)).apply((Applicative)i, NbtContents::new));
/*     */   }
/*     */ 
/*     */   
...
/*  49 */     this(nbtPath, compileNbtPath(nbtPath), interpreting, separator, dataSource);
/*     */   }
/*     */   
...
/*     */     }
/* 118 */     Stream<Tag> elements = this.dataSource.getData(source)
/* 119 */       .flatMap(t -> {
/*     */           try {
...
/* 128 */       RegistryOps<Tag> registryOps = source.registryAccess().createSerializationContext((DynamicOps)net.minecraft.nbt.NbtOps.INSTANCE);
/* 129 */       Component resolvedSeparator = (Component)com.mojang.datafixers.DataFixUtils.orElse(ComponentUtils.updateForEntity(source, this.separator, entity, recursionDepth), ComponentUtils.DEFAULT_NO_STYLE_SEPARATOR);
/* 130 */       return elements.<MutableComponent>flatMap(tag -> {
/*     */             try {
...
/*     */             } 
/* 139 */           }).reduce((left, right) -> left.append(resolvedSeparator).append((Component)right))
/* 140 */         .orElseGet(Component::empty);
/*     */     } 
/* 142 */     Stream<String> stringElements = elements.map(NbtContents::asString);
/* 143 */     return ComponentUtils.updateForEntity(source, this.separator, entity, recursionDepth)
/* 144 */       .map(resolvedSeparator -> (MutableComponent)stringElements.map(Component::literal).reduce(()).orElseGet(Component::empty))
/*     */       
/* 146 */       .orElseGet(() -> Component.literal(stringElements.collect(Collectors.joining(", "))));
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 154 */     if (tag instanceof StringTag) { StringTag stringTag = (StringTag)tag; try { String str1 = stringTag.value(), value = str1;
/* 155 */         return value; } catch (Throwable throwable) { throw new MatchException(throwable.toString(), throwable); }
/*     */        }
...
/* Location:              /home/carlos/.minecraft/versions/1.21.11_unobfuscated/1.21.11_unobfuscated.jar!/net/minecraft/network/chat/contents/NbtContents.class
